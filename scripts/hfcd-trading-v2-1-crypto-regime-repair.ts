...
type ParamSet = {
  ...
  exit_mode: 'fixed' | 'score_decay' | 'opposite_or_decay' | 'profit_trailing';
...
const VERSION = 'HFCD_Trading_V2_1_CryptoRegimeRepair_ModelTrain';
const ROOT = process.cwd();
const OUT_DIR = path.join(ROOT, 'outputs', 'hfcd_trading_v2_1_crypto_regime_repair');
  ...
  while (index < Math.min(endIndex, rows.length - params.holding_bars - 2)) {
    ...
    for (let j = index + 2; j <= Math.min(index + 1 + params.holding_bars, rows.length - 1); j += 1) {
      ...
      if (signal.action === 'long') {
        ...
        if (params.exit_mode === 'profit_trailing' && peakReturn >= params.trail_activate_pct) trailingActive = true;
      ...
      if (params.exit_mode === 'score_decay' || params.exit_mode === 'opposite_or_decay') {
  ...
  for (const fast_bars of [6]) {
    for (const mid_bars of [36]) {
      for (const long_bars of [144, 288]) {
        for (const holding_bars of [12, 24, 48]) {
          for (const min_score of [1.2, 1.55]) {
            for (const min_property_score of [0.62, 0.66]) {
              for (const min_q of [0.64, 0.72]) {
                for (const min_cavity of [0.50]) {
                  for (const max_b_sigma of [0.58, 0.72]) {
                    for (const max_eta of [0.62, 0.78]) {
                      for (const stop_loss_pct of [0.006, 0.012]) {
                        for (const take_profit_pct of [0.018, 0.036]) {
                          for (const trail_activate_pct of [0.012]) {
                            for (const trail_giveback_pct of [0.006]) {
                              for (const cooldown_bars of [3]) {
                                for (const side_policy of ['both', 'long_only', 'short_only'] as const) {
                                  for (const exit_mode of ['fixed', 'score_decay', 'opposite_or_decay', 'profit_trailing'] as const) {
...
async function writeOutputs(summaryRows: Record<string, unknown>[], selectedParams: Record<string, unknown>[], allTrades: Trade[], failureRows: Record<string, unknown>[], coverageRows: Record<string, unknown>[], topRows: Record<string, unknown>[]) {
  ...
  await writeFile(path.join(OUT_DIR, 'hfcd_trading_v2_1_summary.csv'), toCsv(summaryRows));
  await writeFile(path.join(OUT_DIR, 'hfcd_trading_v2_1_selected_params.csv'), toCsv(selectedParams));
  await writeFile(path.join(OUT_DIR, 'hfcd_trading_v2_1_trades.csv'), toCsv(allTrades as any));
  await writeFile(path.join(OUT_DIR, 'hfcd_trading_v2_1_failure_modes.csv'), toCsv(failureRows));
  await writeFile(path.join(OUT_DIR, 'hfcd_trading_v2_1_sensor_coverage.csv'), toCsv(coverageRows));
  await writeFile(path.join(OUT_DIR, 'hfcd_trading_v2_1_top_candidates.csv'), toCsv(topRows));
  await writeFile(path.join(OUT_DIR, 'HFCD_Trading_V2_1_CryptoRegimeRepair_ModelTrain.svg'), equitySvg(allTrades));
  const summary = {
    version: VERSION,
  ...
  await writeFile(path.join(OUT_DIR, 'hfcd_trading_v2_1_summary.json'), JSON.stringify(summary, null, 2));
...
- hfcd_trading_v2_1_summary.json
- hfcd_trading_v2_1_summary.csv
- hfcd_trading_v2_1_selected_params.csv
- hfcd_trading_v2_1_trades.csv
- hfcd_trading_v2_1_failure_modes.csv
- hfcd_trading_v2_1_sensor_coverage.csv
- hfcd_trading_v2_1_top_candidates.csv
- HFCD_Trading_V2_1_CryptoRegimeRepair_ModelTrain.svg
`;
  await writeFile(path.join(OUT_DIR, 'HFCD_Trading_V2_1_CryptoRegimeRepair_ModelTrain.md'), md);
}
