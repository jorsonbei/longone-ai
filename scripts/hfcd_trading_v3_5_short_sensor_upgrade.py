#!/usr/bin/env python3
"""HFCD Trading V3.5: short sensor upgrade.

Local research only. This script does not touch online pages, Binance keys,
testnet accounts, D1 ledgers, or real orders.

Goal:
- Re-test the V3.1 online route set with short-only policies.
- Keep the long-only promoted routes untouched unless short-only also passes.
- Add short-side-specific filters for trend breakdown and crowded fade setups.
...
ROOT = Path.cwd()
OUT_DIR = ROOT / "outputs" / "hfcd_trading_v3_5_short_sensor_upgrade"
...
V16_MACRO_PATH = ROOT / "outputs" / "hfcd_trading_v1_16_gold_full_acquisition" / "hfcd_trading_v1_16_macro_dxy_vix.csv"

SENSOR_DAY: dict[str, dict[str, float]] = {}
SENSOR_HOUR: dict[str, dict[str, float]] = {}
SENSOR_COVERAGE: dict[str, dict[str, int]] = {}
...
def zscore_series(values: list[float], window: int = 90) -> list[float]:
    out: list[float] = []
    hist: list[float] = []
    for value in values:
        hist.append(value)
        ...
        if len(win) < 8:
            ...
            continue
        m = sum(win) / len(win)
        s = math.sqrt(sum((x - m) ** 2 for x in win) / max(len(win) - 1, 1))
        out.append(max(-4.0, min(4.0, (value - m) / max(s, 1e-9))))
    return out


def hour_key(value: Any) -> str:
    try:
        ts = pd.Timestamp(value).tz_convert("UTC") if pd.Timestamp(value).tzinfo else pd.Timestamp(value).tz_localize("UTC")
    except Exception:
        ts = pd.Timestamp(value, tz="UTC")
    return ts.floor("h").isoformat().replace("+00:00", "Z")
...
def add_sensor_coverage(symbol: str, key: str, hit: bool) -> None:
    row = SENSOR_COVERAGE.setdefault(symbol, {})
    row[f"{key}_total"] = row.get(f"{key}_total", 0) + 1
    row[f"{key}_hit"] = row.get(f"{key}_hit", 0) + (1 if hit else 0)


def sensor_coverage_rows() -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for symbol in sorted(SENSOR_COVERAGE):
        counters = SENSOR_COVERAGE[symbol]
        out: dict[str, Any] = {"symbol": symbol}
        keys = sorted({key.removesuffix("_total") for key in counters if key.endswith("_total")})
        for key in keys:
            total = int(counters.get(f"{key}_total", 0))
            hit = int(counters.get(f"{key}_hit", 0))
            out[f"{key}_hit"] = hit
            out[f"{key}_total"] = total
            out[f"{key}_coverage"] = round(hit / total, 6) if total else 0.0
        rows.append(out)
    return rows


def load_short_sensors() -> None:
    """Load local real/proxy sensors for short-side experiments.

    Coverage is intentionally audited. Missing sensors do not block the run, but
    they cannot be counted as evidence for promotion.
    """
    SENSOR_DAY.clear()
    SENSOR_HOUR.clear()
    SENSOR_COVERAGE.clear()

    stable_rows = read_csv_safe(V27_DIR / "hfcd_trading_v2_7_stablecoin_supply_history.csv")
    by_day: dict[str, dict[str, float]] = {}
    for row in stable_rows:
        day = row.get("date", "")
        if not day:
            continue
        item = by_day.setdefault(day, {"stable_d1": 0.0, "stable_d7": 0.0, "stable_supply": 0.0})
        item["stable_d1"] += n(row.get("supply_change_1d_usd"), 12)
        item["stable_d7"] += n(row.get("supply_change_7d_usd"), 12)
        item["stable_supply"] += n(row.get("supply_usd"), 12)
    days = sorted(by_day)
    z1 = zscore_series([by_day[d]["stable_d1"] for d in days])
    z7 = zscore_series([by_day[d]["stable_d7"] for d in days])
    for idx, day in enumerate(days):
        entry = SENSOR_DAY.setdefault(day, {})
        entry["stable_outflow_z"] = max(-4.0, min(4.0, -(0.45 * z1[idx] + 0.55 * z7[idx])))
        entry["stable_supply_usd"] = by_day[day]["stable_supply"]

    macro_rows = read_csv_safe(V16_MACRO_PATH)
    vix_by_day: dict[str, float] = {}
    for row in macro_rows:
        if row.get("symbol") == "VIXCLS":
            vix_by_day[row.get("ts", "")] = n(row.get("value"), 12)
    mdays = sorted(k for k, v in vix_by_day.items() if v > 0)
    vz = zscore_series([vix_by_day[d] for d in mdays])
    last_vix = 0.0
    for idx, day in enumerate(mdays):
        ...
        entry["vix_level"] = vix
        entry["vix_z"] = vz[idx]
        entry["vix_rise"] = max(-4.0, min(4.0, (vix - last_vix) / max(last_vix, 1.0) * 10.0)) if last_vix else 0.0
        last_vix = vix

    funding_rows = read_csv_safe(V24_DIR / "hfcd_trading_v2_4_funding_history.csv")
    for row in funding_rows:
        symbol = row.get("symbol", "")
        key = f"{symbol}|{hour_key(row.get('ts'))}"
        entry = SENSOR_HOUR.setdefault(key, {})
        rate = n(row.get("funding_rate"), 12)
        entry["funding_rate"] = rate
        entry["funding_overheat"] = max(0.0, min(1.0, rate / 0.00010))

    oi_rows = read_csv_safe(V24_DIR / "hfcd_trading_v2_4_open_interest_history.csv")
    last_oi: dict[str, float] = {}
    for row in oi_rows:
        symbol = row.get("symbol", "")
        key = f"{symbol}|{hour_key(row.get('ts'))}"
        oi = n(row.get("open_interest"), 12)
        prev = last_oi.get(symbol, oi)
        entry = SENSOR_HOUR.setdefault(key, {})
        entry["open_interest"] = oi
        entry["oi_expansion"] = max(-1.0, min(1.0, (oi / max(prev, 1e-9) - 1.0) * 25.0))
        last_oi[symbol] = oi

    l2_rows = read_csv_safe(V27_DIR / "hfcd_trading_v2_7_book_depth_5m.csv")
    for row in l2_rows:
        symbol = row.get("symbol", "")
        key = f"{symbol}|{hour_key(row.get('ts'))}"
        entry = SENSOR_HOUR.setdefault(key, {})
        # Negative imbalance means asks dominate bids, useful for short pressure.
        entry["l2_ask_wall"] = max(0.0, min(1.0, -n(row.get("depth_imbalance_0p2"), 12)))
        entry["l2_cavity_usd"] = max(entry.get("l2_cavity_usd", 0.0), n(row.get("liquidity_cavity_1p0_usd"), 12))

    liq_rows = read_csv_safe(V24_DIR / "hfcd_trading_v2_4_liquidation_history.csv")
    for row in liq_rows:
        symbol = row.get("symbol", "")
        key = f"{symbol}|{hour_key(row.get('ts'))}"
        entry = SENSOR_HOUR.setdefault(key, {})
        side = str(row.get("side", "")).lower()
        notional = n(row.get("notional_usd"), 12)
        if "buy" in side or "short" in side:
            entry["short_liquidation_usd"] = entry.get("short_liquidation_usd", 0.0) + notional
        else:
            entry["long_liquidation_usd"] = entry.get("long_liquidation_usd", 0.0) + notional


def fetch_binance_klines(symbol: str, timeframe: str, limit: int = 1500) -> pd.DataFrame:
    interval = {"15m": "15m", "30m": "30m", "1h": "1h"}[timeframe]
    # Use deeper 1h history for crypto shorts so blind tests include more than one regime.
    if timeframe == "1h":
        rows = []
        end_ms = int(datetime.now(timezone.utc).timestamp() * 1000)
        start_ms = int((datetime.now(timezone.utc) - timedelta(days=730)).timestamp() * 1000)
        while end_ms > start_ms and len(rows) < 12000:
            qs = urllib.parse.urlencode({"symbol": symbol, "interval": interval, "limit": limit, "endTime": end_ms})
            ...
            if first_open <= start_ms:
                break
            time.sleep(0.08)
        # De-duplicate after backward pagination.
        seen = set()
        deduped = []
        for row in rows:
            if row[0] in seen or int(row[0]) < start_ms:
                continue
            seen.add(row[0])
            deduped.append(row)
        rows = deduped
...
def fetch_yahoo_chart(symbol: str, timeframe: str) -> pd.DataFrame:
    ...
    # 1h supports longer Yahoo history, which is essential for short-side blind tests.
    lookback = "730d" if timeframe == "1h" else "60d"
    qs = urllib.parse.urlencode({"range": lookback, "interval": interval, "includePrePost": "false"})
...
def enrich(df: pd.DataFrame) -> pd.DataFrame:
    ...
    mom_mid = close.pct_change(32).fillna(0.0)
    ...
    rolling_high = close.rolling(96, min_periods=32).max().bfill()
    rolling_low = close.rolling(96, min_periods=32).min().bfill()
    price_location = ((close - rolling_low) / (rolling_high - rolling_low).replace(0, pd.NA)).fillna(0.5).clip(0.0, 1.0)
    overextension = ((price_location - 0.68) / 0.32).clip(0.0, 1.0)
    red_reversal = ((-ret / (vol_fast * 1.5)).clip(-3.0, 3.0)).apply(sigmoid)
    prior_up_pressure = ((mom_mid + mom_slow) / (vol_slow * 8.0)).clip(-3.0, 3.0).apply(sigmoid)
    volume_exhaustion = ((vol_ratio - 1.0) / 3.0).clip(0.0, 1.0)
    taker_buy_extreme = ((taker_ratio - 0.52) / 0.18).clip(0.0, 1.0)
    short_fade_trigger = (
        0.28 * overextension
        + 0.24 * red_reversal
        + 0.18 * prior_up_pressure
        + 0.16 * volume_exhaustion
        + 0.14 * taker_buy_extreme
    ).clip(0.0, 1.0)
    breakdown_trigger = ((-mom_fast * 0.55 - mom_slow * 0.45) / (vol_fast * 4.0)).clip(-3.0, 3.0).apply(sigmoid)
    ...
    # Short-side repair: combine breakdown shorts with overextended fade shorts.
    out["hfcd_short_score"] = (
        0.58 * short_trend_score
        + 0.24 * short_fade_trigger
        + 0.18 * breakdown_trigger
    ).clip(0.0, 1.0)
    sensor_pressure: list[float] = []
    ...
    asset_class = str(out["asset_class"].iloc[0])
    for _, sensor_row in out.iterrows():
        ts = sensor_row["timestamp"]
        day = day_key(ts)
        hour = hour_key(ts)
        day_sensor = SENSOR_DAY.get(day, {})
        hour_sensor = SENSOR_HOUR.get(f"{symbol}|{hour}", {})
        stable_outflow = max(0.0, min(1.0, (day_sensor.get("stable_outflow_z", 0.0) + 1.5) / 3.0))
        vix_stress = max(0.0, min(1.0, (day_sensor.get("vix_z", 0.0) + day_sensor.get("vix_rise", 0.0) + 1.2) / 3.4))
        funding_overheat = max(0.0, min(1.0, hour_sensor.get("funding_overheat", 0.0)))
        oi_expansion = max(0.0, min(1.0, hour_sensor.get("oi_expansion", 0.0)))
        l2_ask_wall = max(0.0, min(1.0, hour_sensor.get("l2_ask_wall", 0.0)))
        long_liq = hour_sensor.get("long_liquidation_usd", 0.0)
        short_liq = hour_sensor.get("short_liquidation_usd", 0.0)
        liquidation_pressure = max(0.0, min(1.0, math.log1p(long_liq) / math.log1p(10_000_000))) if long_liq or short_liq else 0.0
        if asset_class == "crypto":
            pressure = (
                0.28 * funding_overheat
                + 0.22 * oi_expansion
                + 0.20 * l2_ask_wall
                + 0.18 * stable_outflow
                + 0.12 * liquidation_pressure
            )
            add_sensor_coverage(symbol, "funding", "funding_rate" in hour_sensor)
            add_sensor_coverage(symbol, "open_interest", "open_interest" in hour_sensor)
            add_sensor_coverage(symbol, "l2_depth", "l2_ask_wall" in hour_sensor)
            add_sensor_coverage(symbol, "stablecoin", "stable_outflow_z" in day_sensor)
            add_sensor_coverage(symbol, "liquidation", bool(long_liq or short_liq))
        else:
            # ETF shorting needs market stress confirmation. VIX is real;
            # breadth/credit are still proxies and therefore remain low weight.
            breadth_stress = max(0.0, min(1.0, (-float(ret.loc[sensor_row.name]) / max(float(vol_slow.loc[sensor_row.name]), 1e-6) + 1.0) / 3.0))
            credit_proxy = max(0.0, min(1.0, (vix_stress * 0.55 + breadth_stress * 0.45)))
            pressure = 0.50 * vix_stress + 0.30 * breadth_stress + 0.20 * credit_proxy
            add_sensor_coverage(symbol, "vix", "vix_z" in day_sensor)
            add_sensor_coverage(symbol, "breadth_proxy", True)
            add_sensor_coverage(symbol, "credit_proxy", True)
        sensor_pressure.append(max(0.0, min(1.0, pressure)))
        stable_values.append(stable_outflow)
        vix_values.append(vix_stress)
        funding_values.append(funding_overheat)
        oi_values.append(oi_expansion)
        l2_values.append(l2_ask_wall)
        liq_values.append(liquidation_pressure)
    sensor_series = pd.Series(sensor_pressure, index=out.index)
    out["hfcd_short_sensor_pressure"] = sensor_series
    out["hfcd_short_score_raw"] = out["hfcd_short_score"]
    # Missing sensors must not erase a valid base short signal. Treat the
    # short-specific sensor layer as a confirmation boost, not as a replacement.
    out["hfcd_short_score"] = (out["hfcd_short_score"] + 0.18 * sensor_series).clip(0.0, 1.0)
    out["sensor_stable_outflow"] = stable_values
    out["sensor_vix_stress"] = vix_values
    out["sensor_funding_overheat"] = funding_values
    out["sensor_oi_expansion"] = oi_values
    out["sensor_l2_ask_wall"] = l2_values
    out["sensor_liquidation_pressure"] = liq_values
    out["hfcd_short_trend_score"] = short_trend_score
    out["hfcd_short_fade_trigger"] = short_fade_trigger
    out["hfcd_short_breakdown_trigger"] = breakdown_trigger
    out["bar_return"] = ret
...
def build_policies(symbol: str, asset_class: str, timeframe: str) -> list[Policy]:
    if asset_class == "crypto":
        stop_options = [0.014, 0.020] if timeframe != "1h" else [0.020, 0.028]
        tp_options = [0.026, 0.036] if timeframe != "1h" else [0.038, 0.052]
        holds = {"15m": [8], "30m": [8], "1h": [4, 8]}[timeframe]
        min_cavity_options = [0.40]
        min_eta_options = [0.28]
    else:
        stop_options = [0.0045, 0.0065]
        tp_options = [0.010, 0.014]
        holds = {"15m": [12], "30m": [8], "1h": [3, 6]}[timeframe]
        min_cavity_options = [0.38]
        min_eta_options = [0.30]
    policies: list[Policy] = []
    for threshold in [0.52, 0.56, 0.60, 0.64, 0.68]:
        for hold in holds:
            for sl in stop_options:
                for tp in tp_options:
                    for min_cavity in min_cavity_options:
                        for min_eta in min_eta_options:
                            policies.append(
                                Policy(
                                    symbol=symbol,
...
def load_data() -> tuple[dict[tuple[str, str], pd.DataFrame], list[dict[str, Any]]]:
    ...
    for symbol in CRYPTO_SYMBOLS:
        for tf in TARGET_TIMEFRAMES.get(symbol, TIMEFRAMES):
            try:
...
def main() -> None:
    ...
    sensor_coverage = sensor_coverage_rows()
    write_csv(OUT_DIR / "hfcd_trading_v3_5_data_coverage.csv", coverage)
    write_csv(OUT_DIR / "hfcd_trading_v3_5_sensor_coverage.csv", sensor_coverage)
    ...
    png_path = OUT_DIR / "HFCD_Trading_V3_5_ShortSensorUpgrade.png"
    ...
    write_csv(OUT_DIR / "hfcd_trading_v3_5_policy_summary.csv", summaries)
    write_csv(OUT_DIR / "hfcd_trading_v3_5_selected_routes.csv", selected)
    write_csv(OUT_DIR / "hfcd_trading_v3_5_selected_trades.csv", selected_trades)
    (OUT_DIR / "hfcd_trading_v3_5_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    (OUT_DIR / "HFCD_Trading_V3_5_ShortSensorUpgrade.md").write_text(render_report(summary), encoding="utf-8")
