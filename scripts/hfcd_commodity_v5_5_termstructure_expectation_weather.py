#!/usr/bin/env python3
"""HFCD Commodity V5.5: term structure, expectation, and forecast sensor lift.
...
ROOT = Path.cwd()
VERSION = "HFCD_Commodity_V5_5_TermStructureExpectationWeather"
OUT_DIR = ROOT / "outputs" / "hfcd_commodity_v5_5_termstructure_expectation_weather"
...
def target_units(row: pd.Series, policy: ExogenousPolicy) -> int:
    ...
    # V5.5 product guards are feature-driven, not threshold-search knobs:
    # CL gets exposure cut when the finite-curve/inventory guard contradicts
    # the model; HO can add only when heating/curve/crack quality supports it.
    if policy.symbol == "CL=F":
        guard = float(row.get("v55_cl_profit_guard", 0.0))
        if np.sign(desired) != 0 and np.sign(guard) not in {0, np.sign(desired)} and abs(guard) > 0.65:
            if confidence < policy.confidence_threshold + 0.10:
                return 0
            desired = int(np.sign(desired) * max(1, abs(desired) - 1))
...
def make_report(run_summary: dict[str, Any], selected: list[dict[str, Any]], best_by_route: list[dict[str, Any]]) -> str:
    ...
    lines += [
        ...
        "## 下一步行动计划",
        "",
        "若 V5.5 仍未产生稳定 70% 候选，V5.5 应接真实合约期限结构曲线、第三方库存预期差、NOAA/交易时可用天气预报，而不是继续用同类代理微调。",
        "",
    ]
    return "\n".join(lines)
...
def plot_results(selected: list[dict[str, Any]], best_by_route: list[dict[str, Any]], path: Path) -> None:
    ...
    axes[0].set_ylim(0, 1)
    axes[0].set_title("V5.5 blind-test direction hit rate by validation-first route")
...
def main() -> None:
    ...
    write_csv(OUT_DIR / "hfcd_commodity_v5_5_data_coverage.csv", data_coverage)
    summary_df.to_csv(OUT_DIR / "hfcd_commodity_v5_5_policy_summary.csv", index=False)
    write_csv(OUT_DIR / "hfcd_commodity_v5_5_selected_routes.csv", selected, list(summary_df.columns))
    write_csv(OUT_DIR / "hfcd_commodity_v5_5_best_by_route.csv", best_by_route)
    write_csv(OUT_DIR / "hfcd_commodity_v5_5_bar_replay.csv", all_rows)
    write_csv(OUT_DIR / "hfcd_commodity_v5_5_position_events.csv", all_events)

    report_path = OUT_DIR / "HFCD_Commodity_V5_5_TermStructureExpectationWeather.md"
    chart_path = OUT_DIR / "HFCD_Commodity_V5_5_TermStructureExpectationWeather.png"
    run_summary = {
        "version": VERSION,
    ...
    (OUT_DIR / "hfcd_commodity_v5_5_summary.json").write_text(json.dumps(run_summary, ensure_ascii=False, indent=2), encoding="utf-8")
