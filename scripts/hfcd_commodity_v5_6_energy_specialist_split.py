#!/usr/bin/env python3
"""HFCD Commodity V5.6: energy specialist split.

Local research only. No broker calls, no testnet calls, no online page changes.

Goal:
- Do not move NG/RB/CL/HO to forward ledger yet.
- Split HO and CL instead of forcing one shared selector.
- HO: audit 1.5h/2h/2.5h hit-rate candidates and dynamic sizing.
- CL: preserve the stronger V5.4 feature lineage and add only non-destructive
  curve/inventory overlays so V5.5's curve guard cannot veto good CL signals.
- RB/NG: keep as watchlist only until better real exogenous feeds arrive.
...
ROOT = Path.cwd()
VERSION = "HFCD_Commodity_V5_6_EnergySpecialistSplit"
OUT_DIR = ROOT / "outputs" / "hfcd_commodity_v5_6_energy_specialist_split"
...
TERM_STRUCTURE_CONTRACTS = {
    # Current CME/NYMEX active chain around the 2026-05 run window.
    # These are real finite Yahoo contract pages, not continuous front-month
    # proxies, so they add an explicit curve/roll signal to the continuous
    # futures bars used for the trade replay.
    "CL=F": {"front": "CLM26.NYM", "next": "CLN26.NYM"},
    "RB=F": {"front": "RBM26.NYM", "next": "RBN26.NYM"},
    "HO=F": {"front": "HOM26.NYM", "next": "HON26.NYM"},
    "NG=F": {"front": "NGM26.NYM", "next": "NGN26.NYM"},
}
...
def load_weather_basket_features(start_ts: pd.Timestamp, end_ts: pd.Timestamp) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    try:
        ...
        # Trading-time forecast proxy: only current and past hourly weather
        # are used. It approximates a short-horizon forecast by extrapolating
        # the latest HDD/CDD trajectory instead of reading future observations.
        merged["hdd_forecast_6h_proxy"] = (merged["hdd_65_z"] + 0.75 * merged["hdd_65_delta_6h_z"]).clip(-4, 4)
        merged["cdd_forecast_6h_proxy"] = (merged["cdd_65_z"] + 0.75 * merged["cdd_65_delta_6h_z"]).clip(-4, 4)
        merged["ne_hdd_forecast_6h_proxy"] = (
            merged["northeast_hdd_65_z"] + 0.75 * merged["northeast_hdd_65_delta_6h_z"]
        ).clip(-4, 4)
        merged["texas_cdd_forecast_6h_proxy"] = (
            merged["texas_cdd_65_z"] + 0.75 * merged["texas_cdd_65_delta_6h_z"]
        ).clip(-4, 4)
        merged["ng_weather_forecast_pressure"] = (
            merged["hdd_forecast_6h_proxy"].clip(-3, 3) * 0.40
            + merged["cdd_forecast_6h_proxy"].clip(-3, 3) * 0.30
            + merged["ng_weather_pressure"].clip(-3, 3) * 0.30
        )
        ...
        merged["ho_heating_forecast_pressure"] = (
            merged["ne_hdd_forecast_6h_proxy"].clip(-3, 3) * 0.65
            + merged["ho_heating_weather"].clip(-3, 3) * 0.35
        )
        merged["rb_driving_forecast_pressure"] = (
            merged["texas_cdd_forecast_6h_proxy"].clip(-2, 2) * 0.30
            + merged["rb_driving_weather"].clip(-2, 2) * 0.70
        )
...
def load_contract_chain_features() -> tuple[dict[tuple[str, str], pd.DataFrame], list[dict[str, Any]]]:
    """Load finite front/next CME contract pages and derive curve/roll features.

    This is still a public-data research proxy, but it is materially closer to
    a real term-structure sensor than continuous front-month-only bars.
    """
    coverage: list[dict[str, Any]] = []
    base: dict[tuple[str, str], pd.DataFrame] = {}
    for root_symbol, legs in TERM_STRUCTURE_CONTRACTS.items():
        for leg, yahoo_symbol in legs.items():
            try:
                frame = v51.yahoo_chart(yahoo_symbol, "15m", "60d")
                base[(root_symbol, leg)] = frame
                coverage.append(
                    {
    ...
    for root_symbol in TERM_STRUCTURE_CONTRACTS:
        if (root_symbol, "front") not in base or (root_symbol, "next") not in base:
            continue
        for cadence, rule in ROUTE_FREQS:
            front = v51.resample_bars(base[(root_symbol, "front")], cadence, rule)
            nxt = v51.resample_bars(base[(root_symbol, "next")], cadence, rule)
            if front.empty or nxt.empty:
                continue
            front = front[["timestamp", "close", "volume"]].rename(
                columns={"close": "curve_front_close", "volume": "curve_front_volume"}
            )
            nxt = nxt[["timestamp", "close", "volume"]].rename(
                columns={"close": "curve_next_close", "volume": "curve_next_volume"}
            )
            merged = pd.merge_asof(
                front.sort_values("timestamp"),
                nxt.sort_values("timestamp"),
                on="timestamp",
                direction="nearest",
                tolerance=pd.Timedelta("45min"),
            )
            merged["curve_backwardation"] = (
                merged["curve_front_close"] / (merged["curve_next_close"] + 1e-9) - 1.0
            )
            merged["curve_contango"] = -merged["curve_backwardation"]
            merged["curve_spread_abs"] = merged["curve_front_close"] - merged["curve_next_close"]
            merged["curve_backwardation_z"] = zscore(merged["curve_backwardation"], 96)
            merged["curve_backwardation_delta_z"] = zscore(merged["curve_backwardation"].diff(2), 96)
            merged["curve_spread_abs_z"] = zscore(merged["curve_spread_abs"], 96)
            merged["curve_volume_ratio_z"] = zscore(
                np.log1p(merged["curve_front_volume"]) - np.log1p(merged["curve_next_volume"]),
                96,
            )
            merged["curve_roll_yield_proxy"] = (
                merged["curve_backwardation_z"].clip(-3, 3) * 0.65
                + merged["curve_backwardation_delta_z"].clip(-3, 3) * 0.25
                + merged["curve_volume_ratio_z"].clip(-2, 2) * 0.10
            )
            curves[(root_symbol, cadence)] = merged.sort_values("timestamp").reset_index(drop=True)
    return curves, coverage


def attach_term_structure(out: pd.DataFrame, symbol: str, cadence: str, curves: dict[tuple[str, str], pd.DataFrame]) -> pd.DataFrame:
    curve = curves.get((symbol, cadence), pd.DataFrame())
    if curve.empty:
        for col in [
            "curve_front_close",
            "curve_next_close",
    ...
    merged = pd.merge_asof(
        out.sort_values("timestamp"),
        curve.drop(columns=["curve_front_volume", "curve_next_volume"], errors="ignore").sort_values("timestamp"),
        on="timestamp",
        direction="nearest",
        tolerance=pd.Timedelta("45min"),
    )
    for col in [
        ...
        if col not in merged:
            merged[col] = 0.0
        merged[col] = pd.to_numeric(merged[col], errors="coerce").ffill().fillna(0.0)
    return merged
...
def add_inventory_surprise_features(out: pd.DataFrame) -> pd.DataFrame:
    for base in EIA_SURPRISE_VARIABLES:
        ...
        forecast_source = out[forecast_col] if forecast_col in out.columns else pd.Series(0.0, index=out.index)
        forecast = pd.to_numeric(forecast_source, errors="coerce").fillna(0.0)
        out[f"{base}_forecast_change_z"] = zscore(forecast, 96)
        out[f"{base}_forecast_revision_z"] = zscore(forecast.diff(4).fillna(0.0), 96)
    return out
...
def attach_exogenous(
    ...
    out = attach_term_structure(out, symbol, cadence, curves)
    ...
    for col in [
        ...
        "rb_driving_weather",
        "ho_heating_weather",
        "hdd_forecast_6h_proxy",
        "cdd_forecast_6h_proxy",
        "ne_hdd_forecast_6h_proxy",
        "texas_cdd_forecast_6h_proxy",
    ...
    if symbol == "NG=F":
        out["v53_sigma_ledger"] = (
            ...
            + out["ng_weather_pressure"].clip(-3, 3) * 0.16
            + out["ng_weather_forecast_pressure"].clip(-3, 3) * 0.12
            + out["ng_oil_ratio_delta_z_v54"].clip(-2, 2) * 0.12
            + out["curve_roll_yield_proxy"].clip(-3, 3) * 0.08
            + out["NG=F_mom_z"].clip(-2, 2) * 0.18
        )
    elif symbol == "RB=F":
        out["v53_sigma_ledger"] = (
            -out["gasoline_stocks_surprise_z"].clip(-3, 3) * 0.24
            - out["gasoline_stocks_forecast_revision_z"].clip(-2, 2) * 0.06
            + out["rb_crack_z_v53"].clip(-2, 2) * 0.22
            + out["rb_crack_delta_z_v54"].clip(-2, 2) * 0.13
            + out["driving_season_pressure"] * 0.12
            + out["rb_driving_weather"].clip(-2, 2) * 0.06
            + out["rb_driving_forecast_pressure"].clip(-2, 2) * 0.08
            + out["refinery_demand"].clip(-2, 2) * 0.10
            + out["curve_roll_yield_proxy"].clip(-3, 3) * 0.06
            + out["RB=F_mom_z"].clip(-2, 2) * 0.06
        )
    elif symbol == "HO=F":
        out["v53_sigma_ledger"] = (
            -out["distillate_stocks_surprise_z"].clip(-3, 3) * 0.24
            - out["distillate_stocks_forecast_revision_z"].clip(-2, 2) * 0.08
            + out["ho_crack_z_v53"].clip(-2, 2) * 0.20
            + out["ho_crack_delta_z_v54"].clip(-2, 2) * 0.12
            + out["ho_heating_weather"].clip(-3, 3) * 0.10
            + out["ho_heating_forecast_pressure"].clip(-3, 3) * 0.12
            + out["brent_wti_spread_delta_z_v54"].clip(-2, 2) * 0.08
            + out["curve_roll_yield_proxy"].clip(-3, 3) * 0.08
            + out["HO=F_mom_z"].clip(-2, 2) * 0.10
        )
    else:
        out["v53_sigma_ledger"] = (
            -out["crude_stocks_surprise_z"].clip(-3, 3) * 0.18
            - out["cushing_stocks_surprise_z"].clip(-3, 3) * 0.14
            - out["crude_stocks_forecast_revision_z"].clip(-2, 2) * 0.08
            + out["refinery_inputs_surprise_z"].clip(-3, 3) * 0.12
            + out["refinery_demand"].clip(-2, 2) * 0.14
            + out["brent_wti_spread_z_v53"].clip(-2, 2) * 0.08
            + out["brent_wti_spread_delta_z_v54"].clip(-2, 2) * 0.10
            + out["curve_roll_yield_proxy"].clip(-3, 3) * 0.12
            + out["CL=F_mom_z"].clip(-2, 2) * 0.14
        )

    out["v55_cl_profit_guard"] = (
        out["curve_roll_yield_proxy"].clip(-3, 3) * 0.35
        - out["crude_stocks_surprise_z"].clip(-3, 3) * 0.22
        - out["cushing_stocks_surprise_z"].clip(-3, 3) * 0.18
        + out["brent_wti_spread_delta_z_v54"].clip(-2, 2) * 0.15
        + out["CL=F_mom_z"].clip(-2, 2) * 0.10
    )
    out["v55_ho_size_score"] = (
        -out["distillate_stocks_surprise_z"].clip(-3, 3) * 0.25
        + out["ho_heating_forecast_pressure"].clip(-3, 3) * 0.28
        + out["ho_crack_delta_z_v54"].clip(-2, 2) * 0.20
        + out["curve_roll_yield_proxy"].clip(-3, 3) * 0.17
        + out["HO=F_mom_z"].clip(-2, 2) * 0.10
    )
...
def build_feature_cache() -> tuple[dict[tuple[str, str], pd.DataFrame], list[dict[str, Any]], dict[str, Any]]:
    ...
    curves, curve_coverage = load_contract_chain_features()
    ...
    for cadence, rule in ROUTE_FREQS:
        ...
        for symbol in TARGET_SYMBOLS:
            ...
            feat = attach_exogenous(feat, symbol, cadence, peers, supply, ng_storage, weather, curves)
            feature_cache[(symbol, cadence)] = feat
    sensor_meta = {
        "petroleum_series": metadata,
        "natural_gas_storage": ng_meta,
        "weather_basket": weather_meta,
        "finite_contract_chain": curve_coverage,
        "feature_note": "EIA petroleum surprise + EIA NG storage surprise + hourly city weather forecast proxy + finite contract curve/roll + crack/spread velocity + peer lag features",
    }
    coverage.extend(curve_coverage)
...
def feature_columns(symbol: str) -> list[str]:
    ...
    if symbol == "NG=F":
        ...
        # CL regressed in V5.5 when finite-curve/weather features were allowed
        # to steer the core model. V5.6 restores the V5.4 feature lineage for
        # CL and keeps V5.5 curve/inventory features only as non-destructive
        # sizing overlays in target_units().
        base_features = [c for c in BASE_FEATURES if c not in V55_SENSOR_FEATURES]
        product = CL_V54_PRODUCT_FEATURES
...
def target_units(row: pd.Series, policy: ExogenousPolicy) -> int:
    ...
    desired = units if p >= 0.5 else -units
    # V5.6 specialist sizing:
    # - CL never lets the V5.5 curve guard veto a V5.4-style signal. The guard
    #   can only add exposure when it agrees with the model and confidence is
    #   already high.
    # - HO keeps the V5.5 quality-aware sizing audit because that branch improved
    #   hit-rate evidence, but the result is still selected by validation/test.
    if policy.symbol == "CL=F":
        guard = float(row.get("v55_cl_profit_guard", 0.0))
        if (
            abs(guard) > 1.05
            and np.sign(guard) == np.sign(desired)
            and confidence >= policy.confidence_threshold + 0.07
        ):
            desired = int(np.sign(desired) * min(policy.max_units, max(abs(desired), 2)))
    if policy.symbol == "HO=F":
        size_score = float(row.get("v55_ho_size_score", 0.0))
        if abs(desired) > 1 and abs(size_score) < 0.30:
            desired = int(np.sign(desired))
        if abs(size_score) > 1.15 and np.sign(size_score) == np.sign(desired) and confidence >= policy.confidence_threshold + 0.06:
            desired = int(np.sign(desired) * min(policy.max_units, max(abs(desired), 2)))
    return desired
            ...
            for consensus in ([2, 3] if model_name == "ensemble" else [1]):
                for quality in quality_gates:
                    if symbol == "HO=F":
                        ...
                        # RB/NG remain diagnostics in V5.6. Avoid promoting
                        # fragile performance through leverage.
                        unit_grid = [1]
                    for max_units in unit_grid:
...
def select_routes(summary_df: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    for (symbol, cadence), group in summary_df.groupby(["symbol", "cadence"], sort=False):
        # V5.6 is a specialist split, not a broad promotion pass:
        # - HO can be selected from the 1.5h/2h/2.5h hit-rate branch.
        # - CL must beat a stricter PF floor because V5.4 is already a strong
        #   PF/PnL benchmark.
        # - RB/NG are kept in best_by_route only until better exogenous feeds
        #   arrive.
        if symbol == "HO=F" and cadence not in {"1.5h", "2h", "2.5h"}:
            continue
        if symbol not in {"HO=F", "CL=F"}:
            continue
        candidates = group[group["target70_status"] == "target70_blind_confirmed"].copy()
        if symbol == "CL=F":
            candidates = candidates[
                (candidates["test_direction_hit_rate"] >= 0.64)
                & (candidates["test_profit_factor"] >= 3.0)
                & (candidates["test_net_pnl_usd"] > 25.0)
            ]
        candidates = candidates.sort_values("selection_score", ascending=False)
        if candidates.empty:
            candidates = group[group["stable65_status"] == "stable65_candidate"].copy()
            if symbol == "CL=F":
                candidates = candidates[
                    (candidates["test_direction_hit_rate"] >= 0.64)
                    & (candidates["test_profit_factor"] >= 3.0)
...
def make_report(run_summary: dict[str, Any], selected: list[dict[str, Any]], best_by_route: list[dict[str, Any]]) -> str:
    lines = [
        ...
        "- V5.6 改为分支专家：HO 单独做 1.5h/2h/2.5h 命中率与动态仓位审计；CL 回退 V5.4 核心特征，只允许 V5.5 曲线/库存作为非破坏性加仓叠加；RB/NG 只观察，不晋级。",
    ...
    lines += [
        ...
        "- CL 不再允许 curve guard 过滤信号；如果 CL 仍不能接近 V5.4 PF/PnL，应保留 V5.4 作为 CL 基线。",
        "- HO 若达到 70% 但样本仍小，下一步应扩历史和做仓位缩放压力测试，而不是直接前向。",
        "",
        "## 下一步行动计划",
        "",
        "若 V5.6 仍无法同时提高命中率和交易密度，下一步应分别处理：HO 扩更长历史与真实天气/取暖需求预期；CL 保留 V5.4 强基线并只加可验证叠加；RB/NG 继续补真实库存预期、天气预报和期限结构，不接前向账本。",
        "",
    ]
    return "\n".join(lines)
...
def plot_results(selected: list[dict[str, Any]], best_by_route: list[dict[str, Any]], path: Path) -> None:
    ...
    axes[0].set_ylim(0, 1)
    axes[0].set_title("V5.6 blind-test direction hit rate by validation-first route")
...
def main() -> None:
    ...
    write_csv(OUT_DIR / "hfcd_commodity_v5_6_data_coverage.csv", data_coverage)
    summary_df.to_csv(OUT_DIR / "hfcd_commodity_v5_6_policy_summary.csv", index=False)
    write_csv(OUT_DIR / "hfcd_commodity_v5_6_selected_routes.csv", selected, list(summary_df.columns))
    write_csv(OUT_DIR / "hfcd_commodity_v5_6_best_by_route.csv", best_by_route)
    write_csv(OUT_DIR / "hfcd_commodity_v5_6_bar_replay.csv", all_rows)
    write_csv(OUT_DIR / "hfcd_commodity_v5_6_position_events.csv", all_events)

    report_path = OUT_DIR / "HFCD_Commodity_V5_6_EnergySpecialistSplit.md"
    chart_path = OUT_DIR / "HFCD_Commodity_V5_6_EnergySpecialistSplit.png"
    run_summary = {
        "version": VERSION,
        ...
        "notes": [
            ...
            "Energy specialist split: HO hit-rate/sizing branch, CL V5.4 lineage with non-destructive overlay, RB/NG diagnostics.",
            "Train-only models, validation-selected thresholds, blind test audit.",
        ],
    }
    (OUT_DIR / "hfcd_commodity_v5_6_summary.json").write_text(json.dumps(run_summary, ensure_ascii=False, indent=2), encoding="utf-8")
