...
const VERSION = 'HFCD_Trading_V2_3_CryptoV19HeritageContrast';
const ROOT = process.cwd();
const OUT_DIR = path.join(ROOT, 'outputs', 'hfcd_trading_v2_3_crypto_v19_heritage_contrast');
...
function paramGrid(symbol: SymbolName): ParamSet[] {
  const base = {
    ...
    trail_activate_pct: 0.014,
    trail_giveback_pct: 0.008,
    cooldown_bars: 3,
    side_policy: 'both' as const,
    mode: 'trend_cavity' as const,
    source_lineage: 'V1.9 crypto_v18_execution_cavity_ge_0p65_dynamic_soft',
  };
  return [
    {
      ...base,
      policy_name: 'v19_soft_cavity_hold',
      min_score: 0.75,
      min_property_score: 0.58,
      exit_mode: 'fixed' as const,
    },
    {
      ...base,
      policy_name: 'v19_soft_cavity_profit_trailing',
      min_score: 0.75,
      min_property_score: 0.58,
      exit_mode: 'profit_trailing' as const,
    },
    {
      ...base,
      policy_name: 'v19_soft_cavity_opposite_or_decay',
      min_score: 0.75,
      min_property_score: 0.58,
      exit_mode: 'opposite_or_decay' as const,
    },
    {
      ...base,
      policy_name: 'v19_soft_cavity_stricter_entry',
      min_score: 1.15,
      min_property_score: 0.62,
      min_q: 0.62,
      max_eta: 0.78,
      exit_mode: 'profit_trailing' as const,
    },
...
async function writeOutputs(summaryRows: Record<string, unknown>[], selectedParams: Record<string, unknown>[], allTrades: Trade[], failureRows: Record<string, unknown>[], coverageRows: Record<string, unknown>[], topRows: Record<string, unknown>[]) {
  ...
  await writeFile(path.join(OUT_DIR, 'hfcd_trading_v2_3_summary.csv'), toCsv(summaryRows));
  await writeFile(path.join(OUT_DIR, 'hfcd_trading_v2_3_selected_params.csv'), toCsv(selectedParams));
  await writeFile(path.join(OUT_DIR, 'hfcd_trading_v2_3_trades.csv'), toCsv(allTrades as any));
  await writeFile(path.join(OUT_DIR, 'hfcd_trading_v2_3_failure_modes.csv'), toCsv(failureRows));
  await writeFile(path.join(OUT_DIR, 'hfcd_trading_v2_3_sensor_coverage.csv'), toCsv(coverageRows));
  await writeFile(path.join(OUT_DIR, 'hfcd_trading_v2_3_top_candidates.csv'), toCsv(topRows));
  await writeFile(path.join(OUT_DIR, 'HFCD_Trading_V2_3_CryptoV19HeritageContrast_ModelTrain.svg'), equitySvg(allTrades));
  const summary = {
    version: VERSION,
  ...
  await writeFile(path.join(OUT_DIR, 'hfcd_trading_v2_3_summary.json'), JSON.stringify(summary, null, 2));
...
- hfcd_trading_v2_3_summary.json
- hfcd_trading_v2_3_summary.csv
- hfcd_trading_v2_3_selected_params.csv
- hfcd_trading_v2_3_trades.csv
- hfcd_trading_v2_3_failure_modes.csv
- hfcd_trading_v2_3_sensor_coverage.csv
- hfcd_trading_v2_3_top_candidates.csv
- HFCD_Trading_V2_3_CryptoV19HeritageContrast_ModelTrain.svg
`;
  await writeFile(path.join(OUT_DIR, 'HFCD_Trading_V2_3_CryptoV19HeritageContrast_ModelTrain.md'), md);
}
