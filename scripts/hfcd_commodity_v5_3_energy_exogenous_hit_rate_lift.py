#!/usr/bin/env python3
"""HFCD Commodity V5.3: energy exogenous hit-rate lift.
...
ROOT = Path.cwd()
VERSION = "HFCD_Commodity_V5_3_EnergyExogenousHitRateLift"
OUT_DIR = ROOT / "outputs" / "hfcd_commodity_v5_3_energy_exogenous_hit_rate_lift"
...
def attach_exogenous(df: pd.DataFrame, symbol: str, peers: dict[str, pd.DataFrame], supply: pd.DataFrame, ng_storage: pd.DataFrame) -> pd.DataFrame:
    ...
    if symbol == "NG=F":
        out["v53_sigma_ledger"] = (
            out["ng_storage_tightness"].clip(-3, 3) * 0.40
            + out["ng_oil_ratio_z_v53"].clip(-2, 2) * 0.20
            + out["heating_season_pressure"] * 0.12
            + out["cooling_season_pressure"] * 0.08
            + out["NG=F_mom_z"].clip(-2, 2) * 0.20
        )
    elif symbol == "RB=F":
        out["v53_sigma_ledger"] = (
            -out["gasoline_stocks_change_z"].clip(-3, 3) * 0.30
            + out["rb_crack_z_v53"].clip(-2, 2) * 0.30
            + out["driving_season_pressure"] * 0.15
            + out["refinery_demand"].clip(-2, 2) * 0.15
            + out["RB=F_mom_z"].clip(-2, 2) * 0.10
        )
    elif symbol == "HO=F":
        out["v53_sigma_ledger"] = (
            -out["distillate_stocks_change_z"].clip(-3, 3) * 0.30
            + out["ho_crack_z_v53"].clip(-2, 2) * 0.30
            + out["heating_season_pressure"] * 0.15
            + out["brent_wti_spread_z_v53"].clip(-2, 2) * 0.10
            + out["HO=F_mom_z"].clip(-2, 2) * 0.15
        )
    else:
        out["v53_sigma_ledger"] = (
            out["inventory_tightness"].clip(-3, 3) * 0.30
            - out["cushing_stocks_change_z"].clip(-3, 3) * 0.16
            + out["refinery_demand"].clip(-2, 2) * 0.18
            + out["brent_wti_spread_z_v53"].clip(-2, 2) * 0.16
            + out["CL=F_mom_z"].clip(-2, 2) * 0.20
        )
...
def build_feature_cache() -> tuple[dict[tuple[str, str], pd.DataFrame], list[dict[str, Any]], dict[str, Any]]:
    ...
    for cadence, rule in ROUTE_FREQS:
        ...
        for symbol in TARGET_SYMBOLS:
            ...
            feat = attach_exogenous(feat, symbol, peers, supply, ng_storage)
            feature_cache[(symbol, cadence)] = feat
    sensor_meta = {
        "petroleum_series": metadata,
        "natural_gas_storage": ng_meta,
        "feature_note": "EIA petroleum + EIA NG storage + calendar/weather proxies + crack/spread/peer lag features",
    }
...
def feature_columns(symbol: str) -> list[str]:
    ...
    if symbol == "NG=F":
        product = ["ng_storage_bcf", "ng_storage_change_z", "ng_storage_deficit_z", "ng_storage_tightness"]
    elif symbol == "RB=F":
        product = ["gasoline_stocks_change_z", "rb_crack_z_v53", "driving_season_pressure", "rb_ho_spread_z"]
    elif symbol == "HO=F":
        product = ["distillate_stocks_change_z", "ho_crack_z_v53", "heating_season_pressure", "rb_ho_spread_z"]
    else:
        product = ["crude_stocks_change_z", "cushing_stocks_change_z", "brent_wti_spread_z_v53", "refinery_demand"]
...
def make_report(run_summary: dict[str, Any], selected: list[dict[str, Any]], best_by_route: list[dict[str, Any]]) -> str:
    lines = [
        ...
        "- 本轮新增外生传感器：EIA天然气库存、EIA发布窗口、季节/天气代理、裂解价差、Brent-WTI、跨品种滞后、HFCD Q/C/Bsigma/Manifest 门。",
    ...
    lines += [
        ...
        "- 如果本轮仍不到 65%-70%，下一步不能继续调阈值，应接更真实的天气、期限结构和库存预期差数据。",
        "",
        "## 下一步行动计划",
        "",
        "若 V5.3 仍未产生稳定 65%-70% 候选，V5.4 应接 NOAA/气温城市篮子、NG/RB/CL/HO 合约期限结构、EIA 预期差或第三方库存预期，而不是继续用价格派生代理。",
        "",
    ]
    return "\n".join(lines)
...
def plot_results(selected: list[dict[str, Any]], best_by_route: list[dict[str, Any]], path: Path) -> None:
    ...
    axes[0].set_ylim(0, 1)
    axes[0].set_title("V5.3 blind-test direction hit rate by validation-first route")
...
def main() -> None:
    ...
    status = "energy_exogenous_candidates" if selected else "energy_exogenous_watchlist_no_candidate"
    target70_count = int(sum(1 for r in selected if r.get("target70_status") == "target70_blind_confirmed"))

    write_csv(OUT_DIR / "hfcd_commodity_v5_3_data_coverage.csv", data_coverage)
    summary_df.to_csv(OUT_DIR / "hfcd_commodity_v5_3_policy_summary.csv", index=False)
    write_csv(OUT_DIR / "hfcd_commodity_v5_3_selected_routes.csv", selected, list(summary_df.columns))
    write_csv(OUT_DIR / "hfcd_commodity_v5_3_best_by_route.csv", best_by_route)
    write_csv(OUT_DIR / "hfcd_commodity_v5_3_bar_replay.csv", all_rows)
    write_csv(OUT_DIR / "hfcd_commodity_v5_3_position_events.csv", all_events)

    report_path = OUT_DIR / "HFCD_Commodity_V5_3_EnergyExogenousHitRateLift.md"
    chart_path = OUT_DIR / "HFCD_Commodity_V5_3_EnergyExogenousHitRateLift.png"
    run_summary = {
        "version": VERSION,
        ...
        "notes": [
            ...
            "Exogenous sensor lift: EIA petroleum, EIA NG storage, event windows, season/weather proxies, cracks/spreads, HFCD gates.",
            "Train-only models, validation-selected thresholds, blind test audit.",
        ],
    }
    (OUT_DIR / "hfcd_commodity_v5_3_summary.json").write_text(json.dumps(run_summary, ensure_ascii=False, indent=2), encoding="utf-8")
