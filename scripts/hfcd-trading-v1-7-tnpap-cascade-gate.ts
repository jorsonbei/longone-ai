...
type TnpapProfile = Omit<TnpapVector, 'freshness'>;
...
const VERSION = 'HFCD_Trading_V1_7_TNPAP_CascadeGate';
const ROOT = process.cwd();
const OUT_DIR = path.join(ROOT, 'outputs', 'hfcd_trading_v1_7_tnpap_cascade');
const DATA_DIR = path.join(OUT_DIR, 'market_data');
const V16_DATA_DIR = path.join(ROOT, 'outputs', 'hfcd_trading_v1_6_true_feeds', 'market_data');
...
const PROFILES: Record<AssetClass, TnpapProfile> = {
  gold_etf: {
    q_core: 0.08,
    delta_sigma: 0.24,
    c_cavity: 0.08,
...
for (const hold of [6, 12, 24]) {
  for (const threshold of [0.16, 0.24]) {
    for (const min_manifest of [0.36, 0.48]) {
      for (const strict_quantile of [0.86, 0.93]) {
...
async function loadOrFetchYahoo(symbol: string) {
  ...
  const prior = path.join(V16_DATA_DIR, `${safeName(symbol)}.csv`);
  for (const file of [target, prior]) {
    try {
      const raw = await readFile(file, 'utf8');
      const cached = parseCsv(raw);
      if (cached.length >= 300) {
        if (file === prior) await writeFile(target, raw);
        return cached;
      }
    } catch {
      // fetch below
...
async function loadBinanceMetric(symbol: string, kind: 'funding' | 'oi' | 'longshort'): Promise<TimedMetric[]> {
  ...
  const prior = path.join(V16_DATA_DIR, `binance_${symbol}_${kind}.csv`);
  for (const file of [target, prior]) {
    try {
      const raw = await readFile(file, 'utf8');
...
async function loadCftcCot() {
  ...
  const prior = path.join(V16_DATA_DIR, 'cftc_disagg_cot.csv');
  let text = '';
  for (const file of [target, prior]) {
    try {
      text = await readFile(file, 'utf8');
      if (text.length < 1000) throw new Error('short cache');
      if (file === prior) await writeFile(target, text);
      break;
    } catch {
      text = '';
    }
...
function metricAgeHours(rows: TimedMetric[] | undefined, ts: number) {
  const index = latestIndex(rows, ts);
  if (index < 0 || !rows) return 9999;
  return Math.max(0, (ts - new Date(rows[index].ts).getTime()) / 3_600_000);
}
...
function trueFeedPoint(
  ...
  const cotAge = meta.feed_key ? metricAgeHours(cot.get(meta.feed_key), ts) : 0;
  ...
  const vixLevel = priceLevel(exo.get('^VIX'), ts);
  ...
  const cotFreshness = meta.asset_class === 'futures'
    ? clamp(1 - Math.max(0, cotAge - 168) / 720, 0.45, 1)
    : 1;
  const qualityParts = [
    ...
    meta.asset_class === 'futures' ? ((cot.get(meta.feed_key || '')?.length || 0) > 1 ? cotFreshness : 0.52) : 1,
  ...
  return {
    ...
    credit_impulse: credit,
    risk_appetite: riskAppetite,
    feed_quality: mean(qualityParts),
  };
}

function tnpapVector(
  meta: SymbolMeta,
  rows: MarketRow[],
  index: number,
  side: Side,
  ...
  const q_core = clamp(0.18 + trendAgreement * 0.46 + (side * s.r72 > 0 ? 0.12 : 0), 0, 1);
  const c_cavity = clamp(0.30 + sigmoid(s.volumeZ) * 0.28 - Math.max(0, s.vol - 0.018) * 6 + f.feed_quality * 0.20, 0, 1);
  const r_radius = clamp(sigmoid((s.radius - 2.5) / 1.4), 0, 1);
  const eta_freedom = clamp(sigmoid((s.vol - 0.011) * 95 + Math.abs(s.z72) * 0.12 + Math.abs(s.r1) * 38), 0, 1);
  const vix = f.vix_level || 18;
  const riskOff = clamp(sigmoid((vix - 18) / 6 + f.vix_change * 8), 0, 1);
  const policyRelief = clamp(sigmoid(f.real_rate_pressure * 45 - f.dollar_pressure * 35), 0, 1);
  const riskAppetite = f.risk_appetite;
  const btc = priceRet(exo.get('BTC-USD'), new Date(rows[index].ts).getTime(), 24);
  const eth = priceRet(exo.get('ETH-USD'), new Date(rows[index].ts).getTime(), 24);
  const cryptoConsensus = clamp(sigmoid((btc + eth) * 18 + s.volumeZ * 0.12), 0, 1);
  const oiHeat = sigmoid(f.open_interest_change * 16 + Math.abs(f.funding) * 3000 + Math.abs(f.long_short_bias) * 1.8 - 0.4);
  const leverageSafe = clamp(1 - oiHeat, 0, 1);
  const cotCrowd = Math.abs(f.cot_net);
  const cotSafe = clamp(1 - cotCrowd * 2.2, 0, 1);
  const basisDirectional = clamp(sigmoid(-side * f.basis * 4), 0, 1);
  const breadthDirectional = clamp(sigmoid(side * f.breadth * 2.2 + (side === 1 ? riskAppetite : riskOff) - 0.4), 0, 1);
  const pi_coherence = clamp(sigmoid(side * (s.r6 * 8 + s.r24 * 12 + s.r72 * 6) + trendAgreement - 0.55), 0, 1);

  if (meta.asset_class === 'gold_etf') {
    const defensiveDemand = clamp(riskOff * 0.32 + policyRelief * 0.36 + sigmoid(-f.dollar_pressure * 30) * 0.18 + cotSafe * 0.10, 0, 1);
    const longDelta = defensiveDemand;
    return {
      q_core,
      delta_sigma: side === 1 ? longDelta : clamp(1 - longDelta + f.dollar_pressure * 8, 0, 1),
      c_cavity,
      pi_coherence,
      sigma_ledger: clamp(side === 1 ? (defensiveDemand * 0.45 + cotSafe * 0.20 + sigmoid(s.volumeZ) * 0.15) : (1 - defensiveDemand + f.dollar_pressure * 10), 0, 1),
      eta_freedom,
      b_sigma: side === 1 ? clamp(riskAppetite * 0.36 + Math.max(0, f.dollar_pressure) * 8, 0, 1) : clamp(riskOff * 0.28 + policyRelief * 0.20, 0, 1),
      r_radius,
      tau_time: clamp(0.44 + side * (s.r24 - s.r168) * 7 + policyRelief * 0.20, 0, 1),
      omega_coupling: clamp(side === 1 ? riskOff * 0.55 + policyRelief * 0.25 : riskAppetite * 0.42 + (1 - policyRelief) * 0.24, 0, 1),
      freshness: f.feed_quality,
    };
  }
  ...
  if (meta.asset_class === 'crypto') {
    ...
    return {
      q_core,
      delta_sigma: side === 1 ? clamp(policyRelief * 0.24 + riskAppetite * 0.34 + leverageSafe * 0.22, 0, 1) : clamp(riskOff * 0.45 + (1 - leverageSafe) * 0.25, 0, 1),
      c_cavity,
      pi_coherence,
      sigma_ledger: side === 1 ? clamp(cryptoConsensus * 0.40 + leverageSafe * 0.28 + sigmoid(s.volumeZ) * 0.12, 0, 1) : clamp((1 - cryptoConsensus) * 0.30 + (crowdedLong ? 0.34 : 0) + riskOff * 0.20, 0, 1),
      eta_freedom,
      b_sigma: clamp((1 - leverageSafe) * 0.34 + riskOff * 0.22 + Math.abs(f.funding) * 850 + Math.abs(f.long_short_bias) * 0.18, 0, 1),
      r_radius,
      tau_time: clamp(0.48 + side * (s.r24 - s.r168) * 5 + (side === 1 ? -Math.abs(f.funding) * 500 : Math.abs(f.funding) * 260), 0, 1),
      omega_coupling: side === 1 ? clamp(cryptoConsensus * 0.54 + riskAppetite * 0.26 + leverageSafe * 0.14, 0, 1) : clamp(riskOff * 0.42 + (1 - cryptoConsensus) * 0.24 + (crowdedLong ? 0.18 : 0), 0, 1),
      freshness: f.feed_quality,
    };
  }
  ...
  if (meta.asset_class === 'equity_etf') {
    return {
      q_core,
      delta_sigma: side === 1 ? clamp(policyRelief * 0.34 + riskAppetite * 0.32 + breadthDirectional * 0.18, 0, 1) : clamp(riskOff * 0.50 + (1 - policyRelief) * 0.18, 0, 1),
      c_cavity,
      pi_coherence,
      sigma_ledger: side === 1 ? clamp(riskAppetite * 0.38 + breadthDirectional * 0.34 + sigmoid(s.volumeZ) * 0.10, 0, 1) : clamp(riskOff * 0.42 + (1 - breadthDirectional) * 0.22, 0, 1),
      eta_freedom,
      b_sigma: side === 1 ? clamp(riskOff * 0.55 + Math.max(0, -f.credit_impulse) * 14, 0, 1) : clamp((1 - riskOff) * 0.18 + breadthDirectional * 0.16, 0, 1),
      r_radius,
      tau_time: clamp(0.45 + side * (s.r24 - s.r168) * 5 + policyRelief * 0.10, 0, 1),
      omega_coupling: side === 1 ? riskAppetite : riskOff,
      freshness: f.feed_quality,
    };
  }
  ...
  return {
    q_core,
    delta_sigma: meta.feed_key === 'gold' ? (side === 1 ? policyRelief : 1 - policyRelief) : clamp(side === 1 ? riskAppetite : riskOff, 0, 1),
    c_cavity,
    pi_coherence,
    sigma_ledger: clamp(cotDirectional * 0.42 + basisDirectional * 0.20 + sigmoid(s.volumeZ) * 0.12, 0, 1),
    eta_freedom,
    b_sigma: clamp((1 - cotSafe) * 0.34 + riskOff * 0.20 + Math.max(0, -f.credit_impulse) * 8, 0, 1),
    r_radius,
    tau_time: clamp(0.46 + side * (s.r6 - s.r72) * 8 + basisDirectional * 0.20, 0, 1),
    omega_coupling: meta.feed_key === 'gold' ? (side === 1 ? riskOff : riskAppetite) : riskAppetite,
    freshness: f.feed_quality,
  };
}

function favorableVector(pv: TnpapVector): TnpapProfile {
  return {
    q_core: pv.q_core,
    delta_sigma: pv.delta_sigma,
    c_cavity: pv.c_cavity,
    pi_coherence: pv.pi_coherence,
    sigma_ledger: pv.sigma_ledger,
    eta_freedom: 1 - pv.eta_freedom,
    b_sigma: 1 - pv.b_sigma,
    r_radius: 1 - pv.r_radius,
    tau_time: pv.tau_time,
    omega_coupling: pv.omega_coupling,
  };
}

function dominantProperty(meta: SymbolMeta, pv: TnpapVector) {
  const profile = PROFILES[meta.asset_class];
  const fav = favorableVector(pv);
  return Object.entries(profile)
    .map(([key, weight]) => ({ key, value: fav[key as keyof TnpapProfile] * weight }))
    .sort((a, b) => b.value - a.value)[0]?.key || 'unknown';
}

function cascadeScore(meta: SymbolMeta, pv: TnpapVector): Omit<CascadeScore, 'pv'> {
  const profile = PROFILES[meta.asset_class];
  const fav = favorableVector(pv);
  const weightSum = Object.values(profile).reduce((sum, value) => sum + value, 0);
  const base = Object.entries(profile).reduce((sum, [key, weight]) => sum + fav[key as keyof TnpapProfile] * weight, 0) / weightSum;
  const backgroundGate = clamp((pv.q_core * 0.26 + pv.delta_sigma * 0.30 + (1 - pv.b_sigma) * 0.24 + pv.tau_time * 0.20) * pv.freshness, 0, 1);
  const emergenceGate = clamp(pv.pi_coherence * 0.42 + pv.sigma_ledger * 0.32 + pv.omega_coupling * 0.26, 0, 1);
  const executionGate = clamp(pv.c_cavity * 0.44 + (1 - pv.r_radius) * 0.28 + (1 - pv.eta_freedom) * 0.28, 0, 1);
  const manifestGate = Math.min(backgroundGate, emergenceGate, executionGate, pv.freshness);
  const assetResonance =
    meta.asset_class === 'crypto'
      ? Math.min(pv.omega_coupling, pv.sigma_ledger, pv.c_cavity, 1 - pv.r_radius, 1 - pv.b_sigma)
      : meta.asset_class === 'gold_etf'
        ? Math.min(pv.delta_sigma, pv.tau_time, pv.c_cavity, 1 - pv.b_sigma)
        : meta.asset_class === 'equity_etf'
          ? Math.min(pv.q_core, pv.pi_coherence, pv.sigma_ledger, pv.omega_coupling, 1 - pv.b_sigma)
          : Math.min(pv.tau_time, pv.sigma_ledger, pv.c_cavity, 1 - pv.r_radius);
  const resonance = clamp(assetResonance, 0, 1);
  const hardGate =
    pv.freshness < 0.50 || pv.c_cavity < 0.28 || pv.r_radius > 0.88 || pv.eta_freedom > 0.92 || pv.b_sigma > 0.90
      ? 0
      : clamp(0.44 + backgroundGate * 0.18 + emergenceGate * 0.20 + executionGate * 0.24 + pv.freshness * 0.12, 0, 1);
  const score = base * (0.82 + resonance * 0.58 + backgroundGate * emergenceGate * executionGate * 0.28) * hardGate;
  return {
    score,
    background_gate: backgroundGate,
    emergence_gate: emergenceGate,
    execution_gate: executionGate,
    manifest_gate: manifestGate,
    resonance,
    dominant_property: dominantProperty(meta, pv),
  };
}

function failureMode(c: CascadeScore, threshold: number, minManifest: number) {
  const pv = c.pv;
  if (pv.freshness < 0.50) return '真实物性源过期或不足';
  if (c.background_gate < 0.32) return '低频背景海不相干';
  if (c.emergence_gate < 0.30) return '中频涌现层不确认';
  if (c.execution_gate < 0.30) return '高频执行腔不足';
  if (pv.c_cavity < 0.28) return '熔断：流动性腔体枯竭';
  if (pv.r_radius > 0.88) return '熔断：半径/拥挤越界';
  if (pv.eta_freedom > 0.92) return '熔断：噪声自由度过高';
  if (pv.b_sigma > 0.90) return '熔断：黑子风险过高';
  if (c.manifest_gate < minManifest) return '显化门不足';
  if (c.score < threshold) return '物性融合分数不足';
  return '通过';
}
...
function scoreSide(
  ...
  const pv = tnpapVector(meta, rows, index, side, exo, binance, cot);
  return { pv, ...cascadeScore(meta, pv) };
}
...
function simulate(
  ...
  while (i < rows.length - cfg.hold - 1) {
    ...
    const mode = failureMode(chosen, dynamicThreshold, cfg.min_manifest);
    if (chosen.score < dynamicThreshold || chosen.manifest_gate < cfg.min_manifest || mode !== '通过') {
      const reason = chosen.manifest_gate < cfg.min_manifest ? '显化门不足' : mode;
    ...
    trades.push({
      ...
      score: chosen.score,
      background_gate: chosen.background_gate,
      emergence_gate: chosen.emergence_gate,
      execution_gate: chosen.execution_gate,
      manifest_gate: chosen.manifest_gate,
      resonance: chosen.resonance,
      dominant_property: chosen.dominant_property,
      q_core: chosen.pv.q_core,
      delta_sigma: chosen.pv.delta_sigma,
      c_cavity: chosen.pv.c_cavity,
      pi_coherence: chosen.pv.pi_coherence,
      sigma_ledger: chosen.pv.sigma_ledger,
      eta_freedom: chosen.pv.eta_freedom,
      b_sigma: chosen.pv.b_sigma,
      r_radius: chosen.pv.r_radius,
      tau_time: chosen.pv.tau_time,
      omega_coupling: chosen.pv.omega_coupling,
      freshness: chosen.pv.freshness,
      failure_mode: '通过',
...
function summarize(trades: Trade[]) {
  ...
  return {
    ...
    avg_score: mean(trades.map((trade) => trade.score)),
    avg_manifest: mean(trades.map((trade) => trade.manifest_gate)),
    avg_resonance: mean(trades.map((trade) => trade.resonance)),
    avg_freshness: mean(trades.map((trade) => trade.freshness)),
  };
}
...
function objective(s: ReturnType<typeof summarize>) {
  ...
  return (
    ...
    s.avg_manifest * 450 +
    s.avg_resonance * 350 -
    Math.abs(s.max_drawdown_usd) * 0.55
  );
}
...
async function main() {
  ...
  for (const assetClass of ['gold_etf', 'futures', 'crypto', 'equity_etf'] as const) {
    ...
    for (let fold = 1; fold <= 4; fold += 1) {
      ...
      console.log(
        `[${assetClass}] fold=${fold} trades=${test.trades} pnl=${test.net_pnl_usd.toFixed(0)} pf=${test.profit_factor.toFixed(2)} win=${pct(test.win_rate)} manifest=${test.avg_manifest.toFixed(2)} threshold=${best.threshold.toFixed(3)}`,
      );
      walkRows.push({
        asset_class: assetClass,
        fold,
        ...
        test_avg_score: test.avg_score,
        test_avg_manifest: test.avg_manifest,
        test_avg_resonance: test.avg_resonance,
        test_avg_freshness: test.avg_freshness,
      });
    }
    ...
    for (const trade of wfTrades.slice(0, 2500)) {
      propertyRows.push({
        ...
        score: trade.score,
        background_gate: trade.background_gate,
        emergence_gate: trade.emergence_gate,
        execution_gate: trade.execution_gate,
        manifest_gate: trade.manifest_gate,
        resonance: trade.resonance,
        dominant_property: trade.dominant_property,
        q_core: trade.q_core,
        delta_sigma: trade.delta_sigma,
        c_cavity: trade.c_cavity,
        pi_coherence: trade.pi_coherence,
        sigma_ledger: trade.sigma_ledger,
        eta_freedom: trade.eta_freedom,
        b_sigma: trade.b_sigma,
        r_radius: trade.r_radius,
        tau_time: trade.tau_time,
        omega_coupling: trade.omega_coupling,
    ...
    summaryRows.push({
      ...
      avg_resonance: wf.avg_resonance,
      avg_freshness: wf.avg_freshness,
    });
    configs[assetClass] = {
      status,
      walk_forward_trades: wf.trades,
  ...
  await writeFile(path.join(OUT_DIR, 'hfcd_trading_v1_7_summary.csv'), toCsv(summaryRows));
  await writeFile(path.join(OUT_DIR, 'hfcd_trading_v1_7_walk_forward.csv'), toCsv(walkRows));
  await writeFile(path.join(OUT_DIR, 'hfcd_trading_v1_7_failure_modes.csv'), toCsv(failureRows));
  await writeFile(path.join(OUT_DIR, 'hfcd_trading_v1_7_property_samples.csv'), toCsv(propertyRows));
  await writeFile(path.join(OUT_DIR, 'hfcd_trading_v1_7_feed_coverage.csv'), toCsv(feedRows));
  const summary = {
    version: VERSION,
    ...
    cascade_gates: [
      '低频背景海：Q核 + DeltaSigma + 黑子安全 + Tau',
      '中频涌现层：Pi相干 + Sigma账本 + Omega耦合',
      '高频执行腔：C腔 + R半径安全 + Eta噪声安全',
    ],
    summary: summaryRows,
    feed_coverage: feedRows,
    candidate_config: configs,
    deployable_count: summaryRows.filter((row) => row.status === 'tnpap_validation_pass').length,
  ...
  await writeFile(path.join(OUT_DIR, 'hfcd_trading_v1_7_summary.json'), JSON.stringify(summary, null, 2));

  const md = `# ${VERSION}

## 目标

V1.7 将金融资产从普通时间序列升级为 TNPAP 物性状态向量。它不直接问“价格会不会涨”，而是先判断当前资产是否处在可交易的物性显化窗口。

## 10 种物性

- Q核：资产身份/价值锚。
- DeltaSigma 势差：宏观压力、利率、美元、政策势。
- C腔：市场流动性承载能力。
- Pi相干：趋势一致性。
- Sigma账本：资金流、供需、持仓变化。
- Eta自由度：波动噪声和不确定性。
- B_sigma黑子：脆弱性、事件风险、信用/监管风险。
- R半径：拥挤、杠杆、价格扩散程度。
- Tau时间项：carry、期限结构、相位差。
- Omega耦合：跨资产和市场 regime 耦合。

## 三级级联门

1. 低频背景海：Q核、DeltaSigma、黑子安全、Tau，判断大环境是否允许交易。
2. 中频涌现层：Pi相干、Sigma账本、Omega耦合，判断方向和涌现是否成立。
3. 高频执行腔：C腔、R半径安全、Eta噪声安全，判断能不能真实执行。

## 本轮边界

本轮只做本地 walk-forward 验证，不推线上，不接真实下单。

## 结果

| 资产类 | 状态 | 标的 | 交易数 | 胜率 | 方向命中 | 净收益 | 最大回撤 | Profit Factor | 平均显化门 | 平均共振 |
|---|---:|---|---:|---:|---:|---:|---:|---:|---:|---:|
${summaryRows
  .map(
    (row) =>
      `| ${row.asset_class} | ${row.status} | ${row.symbols} | ${row.walk_forward_trades} | ${pct(Number(row.walk_forward_win_rate))} | ${pct(Number(row.walk_forward_direction_accuracy))} | $${Number(row.walk_forward_net_pnl_usd).toFixed(0)} | $${Number(row.walk_forward_max_drawdown_usd).toFixed(0)} | ${Number(row.walk_forward_profit_factor).toFixed(2)} | ${Number(row.avg_manifest).toFixed(3)} | ${Number(row.avg_resonance).toFixed(3)} |`,
  )
  .join('\n')}
...
${failureRows
  .slice(0, 20)
  .map((row) => `- ${row.asset_class}: ${row.reason} = ${row.count}`)
  .join('\n')}

## 判断

如果 V1.7 优于 V1.6，说明“10 物性 + 级联显化门”能改善真实外部物性源的使用效率。如果仍然 blocked，说明下一步不是调权重，而是补更高质量的真实物性源，例如真实链上活跃/稳定币流动性、完整 COT 历史、股票真实上涨家数和新高新低、期货期限结构曲线。
`;
  await writeFile(path.join(OUT_DIR, 'HFCD_Trading_V1_7_TNPAP_CascadeGate.md'), md);
