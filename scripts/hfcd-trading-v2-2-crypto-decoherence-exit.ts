...
type ParamSet = {
  ...
  exit_mode: 'fixed' | 'score_decay' | 'opposite_or_decay' | 'profit_trailing' | 'trailing_with_decay';
...
const VERSION = 'HFCD_Trading_V2_2_CryptoDecoherenceExit_ModelTrain';
const ROOT = process.cwd();
const OUT_DIR = path.join(ROOT, 'outputs', 'hfcd_trading_v2_2_crypto_decoherence_exit');
const SYMBOLS: SymbolName[] = ['BTCUSDT', 'ETHUSDT'];
const INTERVAL = '5m';
const DAYS = Number(process.env.HFCD_CRYPTO_DAYS || 75);
...
function signalAt(rows: FeatureRow[], index: number, params: ParamSet): Signal {
  ...
  if (row.eta_noise > params.max_eta) return fullSignal(row, 'none', absScore, signedScore, propertyScore, 'eta_noise_overheated');
  if (absScore < params.min_score || propertyScore < params.min_property_score) return fullSignal(row, 'none', absScore, signedScore, propertyScore, 'property_score_underthreshold');
  const side = signedScore >= 0 ? 'long' : 'short';
  if (params.side_policy === 'long_only' && side === 'short') return fullSignal(row, 'none', absScore, signedScore, propertyScore, 'side_policy_reject_short');
  if (params.side_policy === 'short_only' && side === 'long') return fullSignal(row, 'none', absScore, signedScore, propertyScore, 'side_policy_reject_long');
  return fullSignal(row, side, absScore, signedScore, propertyScore, '');
}
...
function simulate(symbol: SymbolName, rows: FeatureRow[], params: ParamSet, split: 'train' | 'validation' | 'test', startIndex: number, endIndex: number) {
  ...
  while (index < Math.min(endIndex, rows.length - params.holding_bars - 2)) {
    ...
    for (let j = index + 2; j <= Math.min(index + 1 + params.holding_bars, rows.length - 1); j += 1) {
      ...
      if (params.exit_mode === 'score_decay' || params.exit_mode === 'opposite_or_decay' || params.exit_mode === 'trailing_with_decay') {
        ...
        if (structuralDecay || (params.exit_mode === 'opposite_or_decay' && oppositeSide && !sameSide)) {
          exitIndex = j;
          exitPrice = bar.close;
...
function paramGrid(symbol: SymbolName): ParamSet[] {
  const grid: ParamSet[] = [];
  for (const fast_bars of [6]) {
    for (const mid_bars of [36]) {
      for (const long_bars of [144, 288]) {
        for (const holding_bars of [48]) {
          for (const min_score of [1.45, 1.9]) {
            for (const min_property_score of [0.64]) {
              for (const min_q of [0.68, 0.76]) {
                for (const min_cavity of [0.50]) {
                  for (const max_b_sigma of [0.55, 0.70]) {
                    for (const max_eta of [0.58, 0.74]) {
                      for (const stop_loss_pct of [0.03, 0.045]) {
                        for (const take_profit_pct of [0.024, 0.04]) {
                          for (const trail_activate_pct of [0.014]) {
                            for (const trail_giveback_pct of [0.006, 0.012]) {
                              for (const cooldown_bars of [6]) {
                                for (const side_policy of ['both', 'long_only', 'short_only'] as const) {
                                  for (const exit_mode of ['opposite_or_decay', 'profit_trailing', 'trailing_with_decay'] as const) {
                                    for (const mode of ['trend', 'trend_cavity'] as const) {
                                      if (long_bars <= mid_bars || mid_bars <= fast_bars) continue;
                                      grid.push({
                                        symbol,
                                        fast_bars,
                                        mid_bars,
...
function selectParam(symbol: SymbolName, rows: FeatureRow[]) {
  ...
  for (const params of paramGrid(symbol)) {
    ...
    const stopLossCount = validationTrades.filter((trade) => trade.exit_reason === 'stop_loss').length;
    const stopLossPenalty = stopLossCount / Math.max(validationTrades.length, 1) * 0.85;
    ranked.push({ params, train, validation, score: pfScore + pnlScore - ddPenalty - stabilityPenalty - stopLossPenalty });
  }
...
async function writeOutputs(summaryRows: Record<string, unknown>[], selectedParams: Record<string, unknown>[], allTrades: Trade[], failureRows: Record<string, unknown>[], coverageRows: Record<string, unknown>[], topRows: Record<string, unknown>[]) {
  ...
  await writeFile(path.join(OUT_DIR, 'hfcd_trading_v2_2_summary.csv'), toCsv(summaryRows));
  await writeFile(path.join(OUT_DIR, 'hfcd_trading_v2_2_selected_params.csv'), toCsv(selectedParams));
  await writeFile(path.join(OUT_DIR, 'hfcd_trading_v2_2_trades.csv'), toCsv(allTrades as any));
  await writeFile(path.join(OUT_DIR, 'hfcd_trading_v2_2_failure_modes.csv'), toCsv(failureRows));
  await writeFile(path.join(OUT_DIR, 'hfcd_trading_v2_2_sensor_coverage.csv'), toCsv(coverageRows));
  await writeFile(path.join(OUT_DIR, 'hfcd_trading_v2_2_top_candidates.csv'), toCsv(topRows));
  await writeFile(path.join(OUT_DIR, 'HFCD_Trading_V2_2_CryptoDecoherenceExit_ModelTrain.svg'), equitySvg(allTrades));
  const summary = {
    version: VERSION,
  ...
  await writeFile(path.join(OUT_DIR, 'hfcd_trading_v2_2_summary.json'), JSON.stringify(summary, null, 2));
...
- 修复 OI endpoint 到 fapi 域名；若历史 OI 仍不足，仍按缺失传感器审计处理。
- 增加 max_eta、min_property_score、side_policy、cooldown、动态退场和 profit trailing。
- 目标是减少 V2.0 的噪声交易和亏损 time_exit，而不是上线页面。
...
- hfcd_trading_v2_2_summary.json
- hfcd_trading_v2_2_summary.csv
- hfcd_trading_v2_2_selected_params.csv
- hfcd_trading_v2_2_trades.csv
- hfcd_trading_v2_2_failure_modes.csv
- hfcd_trading_v2_2_sensor_coverage.csv
- hfcd_trading_v2_2_top_candidates.csv
- HFCD_Trading_V2_2_CryptoDecoherenceExit_ModelTrain.svg
`;
  await writeFile(path.join(OUT_DIR, 'HFCD_Trading_V2_2_CryptoDecoherenceExit_ModelTrain.md'), md);
}
