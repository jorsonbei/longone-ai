#!/usr/bin/env python3
"""HFCD Commodity V5.16: lineage-preserved realtime scanner.

Local research only. No broker calls, no testnet calls, no online page changes.

V5.16 keeps the electricity-style runtime objective:
...
This release does not tune thresholds as the main lever. It restores the best
energy lineages and only uses multi-scale realtime windows as confirmation:
- CL=F preserves the V5.4 3h high-PF lineage;
- HO=F preserves the V5.9/V5.6 2h high-hit-rate lineage;
- scan on 1m / 5m bars when data allows;
- compute past 2h/3h/5h/24h lineage context plus short-window confirmation;
- predict future 15m/30m/1h/1.5h/2h/2.5h/3h.
"""
...
ROOT = Path.cwd()
VERSION = "HFCD_Commodity_V5_16_LineagePreservedRealtimeScanner"
OUT_DIR = ROOT / "outputs" / "hfcd_commodity_v5_16_lineage_preserved_realtime_scanner"
...
V516_COLS = [
    "v516_lineage_enabled",
    "v516_lineage_trade_score",
    "v516_lineage_alignment",
    "v516_lineage_context_2h",
    "v516_lineage_context_3h",
    "v516_lineage_context_5h",
    "v516_lineage_context_24h",
    "v516_lineage_noise_gate",
    "v516_lineage_quality_score",
]
...
def add_v516_lineage_features(frame: pd.DataFrame, symbol: str) -> pd.DataFrame:
    """Restore strong CL/HO lineage while keeping realtime scan cadence.

    V5.15 let the fast multi-scale stack dominate. V5.16 turns those windows
    into confirmation only, and gives CL/HO a lineage score that matches the
    older high-quality blind-test routes:
    - CL: V5.4 3h high-PF route.
    - HO: V5.9/V5.6 2h high-hit-rate route.
    """
    out = frame.copy()
    c2h = num(out, "v515_ret_z_2h")
    c3h = num(out, "v515_ret_z_3h")
    c5h = num(out, "v515_ret_z_5h")
    c24h = num(out, "v515_ret_z_24h")
    supply = num(out, "v513_supply_trade_score")
    expectation = num(out, "v514_expectation_trade_score")
    weather = num(out, "v513_weather_trade_pressure")
    multi = num(out, "v515_multiscale_trade_score")
    quality = (0.55 * num(out, "v513_data_quality_score") + 0.45 * num(out, "v514_expectation_data_quality")).clip(0, 1)
    volatility = num(out, "v515_multiscale_volatility_regime").abs()
    noise_gate = (1.0 / (1.0 + 0.20 * volatility)).clip(0.25, 1.0)

    if symbol == "CL=F":
        lineage_enabled = pd.Series(1.0, index=out.index, dtype=float)
        raw = 0.36 * c3h + 0.22 * c5h + 0.16 * c24h + 0.14 * supply + 0.08 * expectation + 0.04 * multi
        align_inputs = [c3h, c5h, c24h, supply, expectation]
    elif symbol == "HO=F":
        lineage_enabled = pd.Series(1.0, index=out.index, dtype=float)
        raw = 0.36 * c2h + 0.20 * c3h + 0.16 * weather + 0.12 * expectation + 0.10 * supply + 0.06 * multi
        align_inputs = [c2h, c3h, weather, expectation, supply]
    else:
        lineage_enabled = pd.Series(0.0, index=out.index, dtype=float)
        raw = pd.Series(0.0, index=out.index, dtype=float)
        align_inputs = [multi]

    signs = pd.concat([np.sign(s).replace(0, np.nan) for s in align_inputs], axis=1)
    alignment = (signs.sum(axis=1).abs() / signs.count(axis=1).clip(lower=1)).fillna(0.0).clip(0, 1)
    lineage_quality = (lineage_enabled * quality * alignment * noise_gate).clip(0, 1)
    trade_score = (raw * (0.42 + 0.58 * alignment) * noise_gate * (0.55 + 0.45 * quality)).clip(-6, 6)

    out["v516_lineage_enabled"] = lineage_enabled
    out["v516_lineage_trade_score"] = trade_score
    out["v516_lineage_alignment"] = alignment
    out["v516_lineage_context_2h"] = c2h
    out["v516_lineage_context_3h"] = c3h
    out["v516_lineage_context_5h"] = c5h
    out["v516_lineage_context_24h"] = c24h
    out["v516_lineage_noise_gate"] = noise_gate
    out["v516_lineage_quality_score"] = lineage_quality
    for col in V516_COLS:
        out[col] = pd.to_numeric(out.get(col, 0.0), errors="coerce").replace([np.inf, -np.inf], np.nan).fillna(0.0)
    return out
...
def load_mode_features_v513(mode_cfg: dict[str, Any]):
    ...
    for symbol, frame in features.items():
        ...
        enriched = add_v516_lineage_features(enriched, symbol)
        out_features[symbol] = enriched
    sensor_meta = dict(sensor_meta)
    sensor_meta["v516_lineage_preserved_realtime"] = {
        "status": "enabled",
        "features": V513_COLS + V514_COLS + V515_COLS + V516_COLS,
        "finite_contract_curves": {k: len(v) for k, v in curves.items()},
        "lookback_minutes": LOOKBACK_MINUTES,
        "note": "Preserves CL 3h and HO 2h lineage while scanning on 1m/5m bars and predicting future 15m..3h horizons.",
    }
    return out_features, coverage + curve_coverage, sensor_meta


def feature_columns_v513(symbol: str) -> list[str]:
    return list(dict.fromkeys(V512_FEATURE_COLUMNS(symbol) + V513_COLS + V514_COLS + V515_COLS + V516_COLS))
...
def signal_state_v513(row: pd.Series, policy) -> dict[str, Any]:
    ...
    lineage_enabled = float(row.get("v516_lineage_enabled", 0.0))
    lineage = float(row.get("v516_lineage_trade_score", 0.0))
    lineage_alignment = float(row.get("v516_lineage_alignment", 0.0))
    lineage_quality = float(row.get("v516_lineage_quality_score", 0.0))
    ...
    lineage_score = 0.014 * float(np.tanh(lineage / 2.0))
    lineage_return = 0.0014 * float(np.tanh(lineage / 2.0))
    if lineage_enabled >= 0.5:
        score = 0.54 * float(state["score"]) + 0.14 * overlay_score + 0.08 * multiscale_score + 0.24 * lineage_score
        expected = 0.70 * float(state["expected_return"]) + 0.12 * overlay_return + 0.06 * multiscale_return + 0.12 * lineage_return
    else:
        score = 0.70 * float(state["score"]) + 0.20 * overlay_score + 0.10 * multiscale_score
        expected = 0.80 * float(state["expected_return"]) + 0.14 * overlay_return + 0.06 * multiscale_return
    direction = 1 if score >= 0 else -1
    agreement = float(state["agreement"])
    if lineage_enabled >= 0.5 and lineage_quality >= 0.18 and abs(lineage) >= 0.75 and np.sign(lineage) == direction:
        agreement = min(1.0, agreement + 0.10 + 0.06 * lineage_alignment)
    if lineage_enabled >= 0.5 and abs(lineage) >= 1.15 and np.sign(lineage) == -direction:
        agreement = max(0.0, agreement - 0.16)
    ...
    state.update(
        {
            ...
            "v516_lineage_enabled": round(lineage_enabled, 6),
            "v516_lineage_trade_score": round(lineage, 6),
            "v516_lineage_alignment": round(lineage_alignment, 6),
            "v516_lineage_quality_score": round(lineage_quality, 6),
        }
    )
    return state
...
def desired_units_v513(row: pd.Series, policy):
    ...
    if lineage_enabled >= 0.5 and abs(lineage) < 0.70:
        return 0, {**state, "reason": "lineage_signal_too_weak"}
    if lineage_enabled >= 0.5 and lineage_quality < 0.32:
        return 0, {**state, "reason": "lineage_quality_low"}
    if lineage_enabled >= 0.5 and lineage_quality < 0.10 and state["confidence"] < policy.dead_zone + 0.030:
        return 0, {**state, "reason": "lineage_quality_low"}
    if (
        lineage_enabled >= 0.5
        and abs(lineage) > 1.00
        and np.sign(lineage) == -state["direction"]
        and state["confidence"] < policy.dead_zone + 0.040
    ):
        return 0, {**state, "reason": "lineage_disagree"}
    if lineage_enabled >= 0.5 and lineage_alignment < 0.34 and state["confidence"] < policy.dead_zone + 0.035:
        return 0, {**state, "reason": "lineage_alignment_low"}
    ...
    if state["confidence"] >= policy.dead_zone + 0.018 and state["expected_move_bps"] >= policy.min_move_bps + 3.0:
        units += 1
    if lineage_enabled >= 0.5 and lineage_quality >= 0.22 and lineage_alignment >= 0.54 and abs(lineage) >= 0.95 and np.sign(lineage) == state["direction"]:
        units += 1
    ...
    if expectation_quality >= 0.45 and abs(expectation) >= 1.3 and np.sign(expectation) == state["direction"]:
        units += 1
    multiscale_ok = multiscale_alignment >= 0.62 and abs(multiscale) >= 1.4 and np.sign(multiscale) == state["direction"]
    lineage_allows_multiscale = lineage_enabled < 0.5 or (lineage_alignment >= 0.48 and abs(lineage) >= 0.70 and np.sign(lineage) == state["direction"])
    if multiscale_ok and lineage_allows_multiscale:
        units += 1
...
def policy_grid_v513(symbol: str, mode: str) -> list[Any]:
    # V5.16 preserves prior strong lineages. CL/HO get bounded lineage grids;
    # RB/NG remain observation-only until their exogenous sensors improve.
    if symbol == "CL=F":
        horizon_sets = ["far", "all", "mid"]
        profiles = [(0.010, 0.56, 2.0, 0.04, 2, 3.1), (0.014, 0.62, 3.0, 0.08, 2, 2.9), (0.018, 0.68, 5.0, 0.12, 2, 2.7)]
    elif symbol == "HO=F":
        horizon_sets = ["mid", "far", "barbell"]
        profiles = [(0.008, 0.54, 2.0, 0.04, 2, 3.2), (0.012, 0.60, 3.0, 0.08, 3, 3.0), (0.016, 0.66, 5.0, 0.12, 3, 2.8)]
    elif symbol == "RB=F":
        horizon_sets = ["near", "mid"]
        profiles = [(0.010, 0.58, 3.0, 0.08, 1, 3.1), (0.014, 0.66, 5.0, 0.14, 1, 2.8)]
    else:
        horizon_sets = ["near", "mid"]
        profiles = [(0.011, 0.60, 3.0, 0.10, 1, 3.4), (0.015, 0.68, 5.0, 0.16, 1, 3.0)]
    out = []
...
def select_routes(summary_df: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    for _symbol, group in summary_df.groupby("symbol", sort=False):
        if _symbol == "CL=F":
            candidates = group[
                (group["validation_active_signal_rate"] >= 0.05)
                & (group["test_active_signal_rate"] >= 0.05)
                & (group["validation_direction_hit_rate"] >= 0.56)
                & (group["test_direction_hit_rate"] >= 0.58)
                & (group["validation_profit_factor"] >= 1.10)
                & (group["test_profit_factor"] >= 1.25)
                & (group["test_net_pnl_usd"] > 0)
                & (group["test_actions_per_day"] >= 1.00)
            ].copy()
        elif _symbol == "HO=F":
            candidates = group[
                (group["validation_active_signal_rate"] >= 0.05)
                ...
                & (group["test_direction_hit_rate"] >= 0.62)
                & (group["validation_profit_factor"] >= 1.05)
...
def make_report(run_summary: dict[str, Any], selected: list[dict[str, Any]], best_rows: list[dict[str, Any]]) -> str:
    lines = [
        ...
        "- V5.16 不继续调阈值；本版验证“强血统保留 + 当前时刻多 horizon 扫描”是否能恢复 V5.4/V5.9 的质量。",
        "- 主血统：CL=F 继承 V5.4 的 3h 高 PF 路线；HO=F 继承 V5.9/V5.6 的 2h 高命中路线。",
        ...
        "- 新增：`v516_lineage_trade_score`，只让多尺度窗口确认、加仓、减仓、反手，不允许替代 CL/HO 主血统。",
    ...
    lines += [
        ...
        "- V5.16 的“预期差”不是外部分析师 consensus；它是由发布前可交易价格、期限结构、成交量和 API 时间窗市场反应构造的 market-implied proxy。",
        "- 多尺度窗口只使用当前时刻以前的数据；长窗口用于背景判断，不代表降低扫描频率。",
        "- `lineage_preserved` 的含义是：扫描频率可以快，但主方向仍由 CL 3h / HO 2h 的历史强血统决定。",
        ...
        "## 下一步行动计划",
        "",
        "如果 V5.16 有路线通过候选门，下一步做 V5.17 只接通过路线的前向影子账本；如果没有通过，下一步应继续把 CL/HO 的主血统单独固化，补真实第三方库存预期/API 预期历史、完整连续期货链和可交易天气预报，不继续围绕阈值打转。",
    ]
    return "\n".join(lines) + "\n"
...
def make_figure(selected: list[dict[str, Any]], best_rows: list[dict[str, Any]], path: Path) -> None:
    ...
    ax.set_title("V5.16 lineage-preserved realtime scanner")
...
def main() -> None:
                ...
                latest_signals.append(
                    {
                        ...
                        "v516_lineage_trade_score": round(float(tail.iloc[0].get("v516_lineage_trade_score", 0.0)), 6),
                        "v516_lineage_quality_score": round(float(tail.iloc[0].get("v516_lineage_quality_score", 0.0)), 6),
                    }
                )
    ...
    run_summary = {
        ...
        "status": "lineage_preserved_realtime_candidates" if selected else "lineage_preserved_realtime_watchlist_only",
        ...
        "outputs": {
            "summary_json": str(OUT_DIR / "hfcd_commodity_v5_16_summary.json"),
            "policy_summary_csv": str(OUT_DIR / "hfcd_commodity_v5_16_policy_summary.csv"),
            "selected_routes_csv": str(OUT_DIR / "hfcd_commodity_v5_16_selected_routes.csv"),
            "best_by_symbol_csv": str(OUT_DIR / "hfcd_commodity_v5_16_best_by_symbol.csv"),
            "trade_replay_csv": str(OUT_DIR / "hfcd_commodity_v5_16_trade_replay.csv"),
            "action_events_csv": str(OUT_DIR / "hfcd_commodity_v5_16_action_events.csv"),
            "coverage_csv": str(OUT_DIR / "hfcd_commodity_v5_16_coverage.csv"),
            "report_md": str(OUT_DIR / "HFCD_Commodity_V5_16_LineagePreservedRealtimeScanner.md"),
            "figure_png": str(OUT_DIR / "HFCD_Commodity_V5_16_LineagePreservedRealtimeScanner.png"),
        },
        "sensor_meta_by_mode": sensor_meta_by_mode,
    }

    (OUT_DIR / "hfcd_commodity_v5_16_summary.json").write_text(json.dumps(run_summary, ensure_ascii=False, indent=2), encoding="utf-8")
    summary_df.to_csv(OUT_DIR / "hfcd_commodity_v5_16_policy_summary.csv", index=False)
    pd.DataFrame(selected).to_csv(OUT_DIR / "hfcd_commodity_v5_16_selected_routes.csv", index=False)
    pd.DataFrame(best).to_csv(OUT_DIR / "hfcd_commodity_v5_16_best_by_symbol.csv", index=False)
    v511.write_csv(OUT_DIR / "hfcd_commodity_v5_16_trade_replay.csv", all_rows)
    v511.write_csv(OUT_DIR / "hfcd_commodity_v5_16_action_events.csv", all_events)
    v511.write_csv(OUT_DIR / "hfcd_commodity_v5_16_coverage.csv", all_coverage)
    (OUT_DIR / "HFCD_Commodity_V5_16_LineagePreservedRealtimeScanner.md").write_text(
        make_report(run_summary, selected, best), encoding="utf-8"
    )
    make_figure(selected, best, OUT_DIR / "HFCD_Commodity_V5_16_LineagePreservedRealtimeScanner.png")
