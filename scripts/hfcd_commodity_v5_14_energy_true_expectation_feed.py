#!/usr/bin/env python3
"""HFCD Commodity V5.14: energy true expectation feed.

Local research only. No broker calls, no testnet calls, no online page changes.

V5.14 keeps the electricity-style runtime objective:
...
- pre-EIA market-implied inventory expectation from tradable price/curve/volume;
- API/EIA expectation proxy windows from pre-release market reaction;
- post-EIA 15m/30m/1h shock-response features;
- finite contract-chain curve features inherited from V5.13.
"""
...
ROOT = Path.cwd()
VERSION = "HFCD_Commodity_V5_14_EnergyTrueExpectationFeed"
OUT_DIR = ROOT / "outputs" / "hfcd_commodity_v5_14_energy_true_expectation_feed"
...
def load_mode_features_v513(mode_cfg: dict[str, Any]):
    ...
    sensor_meta["v514_expectation_feed"] = {
        "status": "enabled",
        "features": V513_COLS + V514_COLS,
        "finite_contract_curves": {k: len(v) for k, v in curves.items()},
        "note": "Uses market-implied and API-proxy expectation windows from tradable pre-release price/curve/volume. It is not an external analyst-consensus feed.",
    }
    return out_features, coverage + curve_coverage, sensor_meta


def feature_columns_v513(symbol: str) -> list[str]:
    return list(dict.fromkeys(V512_FEATURE_COLUMNS(symbol) + V513_COLS + V514_COLS))
...
def signal_state_v513(row: pd.Series, policy) -> dict[str, Any]:
    ...
    score = 0.78 * float(state["score"]) + 0.22 * overlay_score
    expected = 0.84 * float(state["expected_return"]) + 0.16 * overlay_return
...
def policy_grid_v513(symbol: str, mode: str) -> list[Any]:
    # V5.14 is an expectation-feed run. Keep grid bounded and comparable to V5.12/V5.13.
    if symbol == "CL=F":
...
def make_report(run_summary: dict[str, Any], selected: list[dict[str, Any]], best_rows: list[dict[str, Any]]) -> str:
    lines = [
        ...
        "- V5.14 不继续调阈值；本版验证市场隐含预期差、API proxy 和 EIA 发布冲击是否提升方向预测和收益质量。",
    ...
    lines += [
        ...
        "- V5.14 的“预期差”不是外部分析师 consensus；它是由发布前可交易价格、期限结构、成交量和 API 时间窗市场反应构造的 market-implied proxy。",
        ...
        "## 下一步行动计划",
        "",
        "如果 V5.14 有路线通过候选门，下一步做 V5.15 只接通过路线的前向影子账本；如果没有通过，下一步应补真实第三方库存预期/API 预期历史、完整连续期货链和可交易天气预报，不继续围绕阈值打转。",
    ]
    return "\n".join(lines) + "\n"
...
def make_figure(selected: list[dict[str, Any]], best_rows: list[dict[str, Any]], path: Path) -> None:
    ...
    ax.set_title("V5.14 energy true expectation feed")
...
def main() -> None:
    ...
    run_summary = {
        ...
        "status": "energy_true_expectation_candidates" if selected else "energy_true_expectation_watchlist_only",
        ...
        "outputs": {
            "summary_json": str(OUT_DIR / "hfcd_commodity_v5_14_summary.json"),
            "policy_summary_csv": str(OUT_DIR / "hfcd_commodity_v5_14_policy_summary.csv"),
            "selected_routes_csv": str(OUT_DIR / "hfcd_commodity_v5_14_selected_routes.csv"),
            "best_by_symbol_csv": str(OUT_DIR / "hfcd_commodity_v5_14_best_by_symbol.csv"),
            "trade_replay_csv": str(OUT_DIR / "hfcd_commodity_v5_14_trade_replay.csv"),
            "action_events_csv": str(OUT_DIR / "hfcd_commodity_v5_14_action_events.csv"),
            "coverage_csv": str(OUT_DIR / "hfcd_commodity_v5_14_coverage.csv"),
            "report_md": str(OUT_DIR / "HFCD_Commodity_V5_14_EnergyTrueExpectationFeed.md"),
            "figure_png": str(OUT_DIR / "HFCD_Commodity_V5_14_EnergyTrueExpectationFeed.png"),
        },
        "sensor_meta_by_mode": sensor_meta_by_mode,
    }

    (OUT_DIR / "hfcd_commodity_v5_14_summary.json").write_text(json.dumps(run_summary, ensure_ascii=False, indent=2), encoding="utf-8")
    summary_df.to_csv(OUT_DIR / "hfcd_commodity_v5_14_policy_summary.csv", index=False)
    pd.DataFrame(selected).to_csv(OUT_DIR / "hfcd_commodity_v5_14_selected_routes.csv", index=False)
    pd.DataFrame(best).to_csv(OUT_DIR / "hfcd_commodity_v5_14_best_by_symbol.csv", index=False)
    v511.write_csv(OUT_DIR / "hfcd_commodity_v5_14_trade_replay.csv", all_rows)
    v511.write_csv(OUT_DIR / "hfcd_commodity_v5_14_action_events.csv", all_events)
    v511.write_csv(OUT_DIR / "hfcd_commodity_v5_14_coverage.csv", all_coverage)
    (OUT_DIR / "HFCD_Commodity_V5_14_EnergyTrueExpectationFeed.md").write_text(
        make_report(run_summary, selected, best), encoding="utf-8"
    )
    make_figure(selected, best, OUT_DIR / "HFCD_Commodity_V5_14_EnergyTrueExpectationFeed.png")
