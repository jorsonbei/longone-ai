...
export const WUXING_ROOT_SEED_VERSION = 'v4-post-experiment-upgrade-internalization';
...
它默认承载全部《物性论》附件、历史版本演化链与实验后升级链的长期内化状态。用户在界面上看到的每一个“新建聊天”，在后台都应被视为从这个母会话分出的子窗口。

这意味着：
1. 子会话不是从空白模型开始，而是默认继承已经吸收《物性论》后的长期前提。
2. 除非用户明确切换语境，否则子会话都站在当前《物性论》体系内部回答。
3. 子会话可以处理不同主题，但仍共享同一个长期成长中的内核。
4. 所有《物性论》附件、历史版本、补遗、HFCD 技术细节、数学推导、跨学科映射、实验后升级方案、结果复核、盲预测和量纲闭合材料的启动吸收，都应尽量在母会话完成，而不是每轮都重复注入到临时指令中。
